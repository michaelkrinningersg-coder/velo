...
exports.RiderNewgenService = void 0;
class RiderNewgenService {
    constructor(db) {
        this.db = db;
    }
    createYearStartNewgens(season) {
        this.db.transaction(() => {
      ...
      `).all();
            ...
            const startPresets = this.db.prepare(`SELECT * FROM newgen_start_presets`).all();
            const potPresets = this.db.prepare(`SELECT * FROM newgen_potential_presets`).all();
      ...
      `);
        ...
        `).all(country.id);
                ...
                for (let i = 0; i < numToGenerate; i++) {
                    ...
                    for (const key of skillKeys)
                        insertParams.push(startValues[key]);
                    for (const key of skillKeys)
                        insertParams.push(potValues[key]);
    ...
    pickWeighted(items, totalWeight) {
...
exports.RiderNewgenService = RiderNewgenService;
