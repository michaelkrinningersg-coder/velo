...
exports.calculateStageScore = calculateStageScore;
exports.calculateClimbScoresForStage = calculateClimbScoresForStage;
const SCORE_CALIBRATION_DIVISOR = 8.9051;
const STAGE_SCORE_MAX = 1000;
function isClassifiedClimbEnd(marker) {
    return marker.type === 'climb_top'
        || marker.type === 'finish_hill'
        || marker.type === 'finish_mountain';
}
function normalizeClimbCategory(category) {
    return category != null && category !== 'Sprint' ? category : null;
}
function resolveClimbEndCategory(marker, lastSeenCategory) {
...
function buildPositionedScoreSegments(segments, startElevation) {
    let currentKm = 0;
...
function calculateClimbScoresForStage(segments, startElevation) {
    const totalDistanceKm = segments.reduce((sum, segment) => sum + segment.lengthKm, 0);
    const positionedSegments = buildPositionedScoreSegments(segments, startElevation);
    const climbs = [];
    let openClimb = null;
