...
function clamp(v, min = 0, max = RIDER_STAT_MAX) {
...
function parseIdList(value) {
    return value
        ...
        if (!Number.isInteger(parsed))
            throw new Error(`Ungültige race_id-Liste: "${value}".`);
        return parsed;
    });
}
...
const ROLE_NAME_CAPTAIN = 'Kapitaen';
const ROLE_NAME_CO_CAPTAIN = 'Co-Kapitaen';
const ROLE_NAME_ROAD_CAPTAIN = 'Edelhelfer';
const ROLE_NAME_STRONG_HELPER = 'Starke Helfer';
const ROLE_NAME_DOMESTIQUE = 'Wassertraeger';
const ROLE_NAME_SPRINTER = 'Sprinter';
...
function parseCountrySeeds() {
    return readCsv('sta_country.csv').map((row, index) => {
        ...
        if (code3.length !== 3)
            throw new Error(`${ctx}: code_3 muss 3 Zeichen haben.`);
        const numberRegenMin = int(req(row, 'number_regen_min', ctx), ctx);
        const numberRegenMax = int(req(row, 'number_regen_max', ctx), ctx);
        if (numberRegenMax < numberRegenMin)
            throw new Error(`${ctx}: number_regen_max muss >= number_regen_min sein.`);
        return {
            id: int(req(row, 'id', ctx), ctx),
...
function parseRaceCategoryBonusSeeds() {
...
function parseRaceCategorySeeds() {
...
function buildRoleMaps(roleSeeds) {
    const idByName = new Map();
    const weightingByName = new Map();
    for (const role of roleSeeds) {
        if (idByName.has(role.name))
            throw new Error(`Doppelte Rollenbezeichnung ${role.name} in sta_role.csv.`);
    ...
    for (const requiredRoleName of REQUIRED_ROLE_NAMES) {
        if (!idByName.has(requiredRoleName))
            throw new Error(`Pflichtrolle ${requiredRoleName} fehlt in sta_role.csv.`);
    }
    return { idByName, weightingByName };
}
function getRoleScore(rider) {
    return (rider.skills.mountain * 2
        + rider.skills.hill
        + rider.skills.sprint
        + rider.skills.timeTrial
        + rider.skills.cobble) / 6;
}
function getRoundedRoleCount(teamSize, percentage) {
    return Math.max(0, Math.round(teamSize * (percentage / 100)));
}
function compareByRoleRanking(left, right) {
    return getRoleScore(right) - getRoleScore(left)
        || right.skills.sprint - left.skills.sprint
        || left.riderId - right.riderId;
}
function compareBySprintRanking(left, right) {
    return right.skills.sprint - left.skills.sprint
        || getRoleScore(right) - getRoleScore(left)
        || left.riderId - right.riderId;
}
function assignTeamRoleIds(riders, roleSeeds) {
    const { idByName, weightingByName } = buildRoleMaps(roleSeeds);
    const riderRoleIds = new Map();
    ...
    for (const teamRiders of ridersByTeam.values()) {
        ...
        const captainCount = Math.min(teamSize, teamSize >= 25
        ...
        const coCaptainIds = new Set();
        const sprinterIds = new Set();
        ...
        const coCaptainCount = Math.min(teamSize - captainIds.size, teamSize >= 25
        ...
        const helperPool = availableRiders.filter(rider => !captainIds.has(rider.riderId) && !coCaptainIds.has(rider.riderId) && !sprinterIds.has(rider.riderId));
...
function assignRiderType(input, currentSeason) {
    const { skills } = input;
    ...
    const tags = (0, RiderTagService_1.deriveRiderTags)(topThree, skills.recuperation);
    return {
        potentials,
...
function parseRiderSeeds() {
    return readCsv('riders.csv').map((row, index) => {
        const ctx = `riders.csv Zeile ${index + 2}`;
        const importedSkills = Object.fromEntries(RIDER_IMPORTED_SKILL_COLUMNS.map(([key, column]) => [key, clamp(int(req(row, `skill_${column}`, ctx), ctx))]));
        const skills = {
            ...importedSkills,
...
function bootstrap(force = false) {
    ...
    for (const country of countrySeeds) {
        if (country.id <= 0)
            throw new Error(`Country-ID ${country.id} muss positiv sein.`);
        if (seenCountryIds.has(country.id))
            throw new Error(`Doppelte Country-ID ${country.id} in sta_country.csv.`);
        if (seenCountryCodes.has(country.code3))
            throw new Error(`Doppelter code_3 ${country.code3} in sta_country.csv.`);
        seenCountryIds.add(country.id);
        seenCountryCodes.add(country.code3);
    }
    const insCountry = db.prepare('INSERT INTO sta_country (id, name, code_3, continent, regen_rating, number_regen_min, number_regen_max) VALUES (?, ?, ?, ?, ?, ?, ?)');
    for (const country of countrySeeds) {
    ...
    const insRole = db.prepare('INSERT INTO sta_role (id, name, weighting) VALUES (?, ?, ?)');
    for (const role of roleSeeds) {
        if (role.id <= 0)
            throw new Error(`Role-ID ${role.id} muss positiv sein.`);
        if (seenRoleIds.has(role.id))
            throw new Error(`Doppelte Role-ID ${role.id} in sta_role.csv.`);
        seenRoleIds.add(role.id);
        insRole.run(role.id, role.name, role.weighting);
    }
    const seenTypeRiderIds = new Set();
    const seenTypeRiderKeys = new Set();
    const typeRiderIdByKey = new Map();
    const insTypeRider = db.prepare('INSERT INTO type_rider (id, type_key, display_name, is_stage_focus, is_one_day_focus) VALUES (?, ?, ?, ?, ?)');
    for (const typeRider of typeRiderSeeds) {
        if (typeRider.id <= 0)
            throw new Error(`Type-Rider-ID ${typeRider.id} muss positiv sein.`);
        if (seenTypeRiderIds.has(typeRider.id))
            throw new Error(`Doppelte Type-Rider-ID ${typeRider.id} in type_rider.csv.`);
        if (seenTypeRiderKeys.has(typeRider.key))
            throw new Error(`Doppelter rider_type key ${typeRider.key} in type_rider.csv.`);
    ...
    for (const team of teamSeeds) {
        if (team.teamId <= 0)
            throw new Error(`Team-ID ${team.teamId} für Team "${team.name}" muss positiv sein.`);
        if (usedTeamIds.has(team.teamId))
            throw new Error(`Doppelte team_id ${team.teamId} in teams.csv.`);
        if (!seenCountryIds.has(team.countryId))
            throw new Error(`Unbekannte country_id ${team.countryId} für Team "${team.name}".`);
        if (team.divisionName === 'U23')
            throw new Error(`U23-Teams werden nicht mehr unterstützt: "${team.name}".`);
        for (const focusId of [team.aiFocus1, team.aiFocus2, team.aiFocus3]) {
            if (!seenTypeRiderIds.has(focusId))
                throw new Error(`Unbekannte ai_focus-ID ${focusId} für Team "${team.name}".`);
        }
        usedTeamIds.add(team.teamId);
    }
    const insTeam = db.prepare(`INSERT INTO teams (id, name, abbreviation, division_id, u23_team, is_player_team, country_id, color_primary, color_secondary, ai_focus_1, ai_focus_2, ai_focus_3)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
    ...
    const knownTeamIds = new Set(db.prepare('SELECT id FROM teams').all().map(team => team.id));
  ...
  `);
    for (const rider of seededRiders) {
        if (usedRiderIds.has(rider.riderId))
            throw new Error(`Doppelte rider_id ${rider.riderId} in riders.csv.`);
        if (!knownTeamIds.has(rider.activeTeamId))
            throw new Error(`Unbekannte team_id ${rider.activeTeamId} für Fahrer ${rider.firstName} ${rider.lastName}.`);
        if (!seenCountryIds.has(rider.countryId))
            throw new Error(`Unbekannte country_id ${rider.countryId} für Fahrer ${rider.firstName} ${rider.lastName}.`);
        ...
        if (riderTypeId == null)
            throw new Error(`Kein rider_type Mapping für ${rider.riderType} bei Fahrer ${rider.firstName} ${rider.lastName}.`);
        if (rider.specialization1 != null && specialization1Id == null)
            throw new Error(`Kein specialization_1 Mapping für ${rider.specialization1} bei Fahrer ${rider.firstName} ${rider.lastName}.`);
        if (rider.specialization2 != null && specialization2Id == null)
            throw new Error(`Kein specialization_2 Mapping für ${rider.specialization2} bei Fahrer ${rider.firstName} ${rider.lastName}.`);
        if (rider.specialization3 != null && specialization3Id == null)
            throw new Error(`Kein specialization_3 Mapping für ${rider.specialization3} bei Fahrer ${rider.firstName} ${rider.lastName}.`);
        insRider.run(rider.riderId, rider.firstName, rider.lastName, rider.countryId, rider.roleId, rider.birthYear, rider.peakAge, rider.declineAge, rider.retirementAge, rider.skillDevelopment, rider.potential, rider.overallRating, ...skillValues, ...potentialValues, riderTypeId, specialization1Id, specialization2Id, specialization3Id, rider.isStageRacer, rider.isOneDayRacer, rider.hasGrandTourTag, rider.hasStageRaceTag, rider.hasOneDayClassicTag, rider.favoriteRaces, rider.nonFavoriteRaces, null, null, 0);
    }
    const insContract = db.prepare('INSERT INTO contracts (rider_id, team_id, start_season, end_season, status) VALUES (?, ?, ?, ?, ?)');
    ...
    const knownBonusIds = new Set();
    const insRaceCategoryBonus = db.prepare(`INSERT INTO race_categories_bonus (
    ...
    for (const bonus of raceCategoryBonusSeeds) {
        if (bonus.id <= 0)
            throw new Error(`RaceCategoryBonus-ID ${bonus.id} muss positiv sein.`);
        if (knownBonusIds.has(bonus.id))
            throw new Error(`Doppelte race_categories_bonus id ${bonus.id}.`);
        knownBonusIds.add(bonus.id);
        insRaceCategoryBonus.run(bonus.id, bonus.name, bonus.bonusSecondsFinal, bonus.bonusSecondsIntermediate, bonus.pointsStage, bonus.pointsOneDay, bonus.pointsGcFinal, bonus.pointsJerseyLeaderDay, bonus.pointsJerseySprintDay, bonus.pointsJerseyMountainDay, bonus.pointsJerseyYouthDay, bonus.pointsSprintIntermediate, bonus.pointsMountainHc, bonus.pointsMountainCat1, bonus.pointsMountainCat2, bonus.pointsMountainCat3, bonus.pointsMountainCat4, bonus.pointsJerseySprintFinal, bonus.pointsJerseyMountainFinal, bonus.pointsJerseyYouthFinal);
    }
    const knownRaceCategoryIds = new Set();
    const insRaceCategory = db.prepare('INSERT INTO race_categories (id, name, tier, number_of_teams, number_of_riders, bonus_system_id) VALUES (?, ?, ?, ?, ?, ?)');
    for (const category of raceCategorySeeds) {
        if (category.id <= 0)
            throw new Error(`RaceCategory-ID ${category.id} muss positiv sein.`);
        if (knownRaceCategoryIds.has(category.id))
            throw new Error(`Doppelte race_categories id ${category.id}.`);
    ...
    const insRace = db.prepare('INSERT INTO races (id, name, country_id, category_id, is_stage_race, number_of_stages, start_date, end_date, prestige) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)');
    for (const race of raceSeeds) {
        if (race.id <= 0)
            throw new Error(`Race-ID ${race.id} muss positiv sein.`);
        if (knownRaceIds.has(race.id))
            throw new Error(`Doppelte races id ${race.id}.`);
        if (!seenCountryIds.has(race.countryId))
            throw new Error(`Unbekannte country_id ${race.countryId} fuer Rennen ${race.name}.`);
        if (!knownRaceCategoryIds.has(race.categoryId))
            throw new Error(`Unbekannte category_id ${race.categoryId} fuer Rennen ${race.name}.`);
    ...
    const stageCountsByRace = new Map();
    const usedStageIds = new Set();
    const usedStageKeys = new Set();
    const insStage = db.prepare('INSERT INTO stages (id, race_id, stage_number, date, profile, details_csv_file) VALUES (?, ?, ?, ?, ?, ?)');
    for (const stage of stageSeeds) {
        if (stage.id <= 0)
            throw new Error(`Stage-ID ${stage.id} muss positiv sein.`);
        if (usedStageIds.has(stage.id))
            throw new Error(`Doppelte stages id ${stage.id}.`);
        if (!knownRaceIds.has(stage.raceId))
            throw new Error(`Unbekannte race_id ${stage.raceId} in stages.csv.`);
        const stageKey = `${stage.raceId}:${stage.stageNumber}`;
        if (usedStageKeys.has(stageKey))
            throw new Error(`Doppelte Stage-Nummer ${stage.stageNumber} fuer race_id ${stage.raceId}.`);
    ...
    db.prepare('INSERT OR REPLACE INTO game_state (id, "current_date", season, is_game_over) VALUES (1, ?, ?, ?)').run(currentDate, currentSeason, isGameOver);
    console.log(`  Spielzustand gesetzt: ${currentDate}, Saison ${currentSeason}.`);
    // Race-Entries
    const raceRows = db.prepare('SELECT id FROM races').all();
    const riderRows = db.prepare('SELECT id, active_team_id FROM riders WHERE active_team_id IS NOT NULL').all();
    ...
    db.transaction(() => {
        for (const race of raceRows)
            for (const rider of riderRows)
                insEntry.run(race.id, rider.active_team_id, rider.id);
    })();
