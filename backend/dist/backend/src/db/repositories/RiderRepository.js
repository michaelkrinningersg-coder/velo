...
exports.RiderRepository = void 0;
...
class RiderRepository {
    ...
    attachMentorData(riders) {
        ...
        return riders.map((rider) => {
            if ((rider.age ?? 0) <= 23 && rider.activeTeamId != null) {
                ...
                const mentors = teamRiders.filter(m => m.id !== rider.id &&
                    (m.age ?? 0) >= 31 &&
                    m.overallRating >= 73 &&
    ...
    getRiderProgramRaceSummary(riderId) {
        const season = new GameStateRepository_1.GameStateRepository(this.db).getCurrentSeason();
        if (!(0, mappers_1.tableExists)(this.db, 'rider_season_programs') || !(0, mappers_1.tableExists)(this.db, 'race_programs')) {
            return null;
        }
    ...
    `).all(programRow.id);
        const stagesByRaceId = new RaceRepository_1.RaceRepository(this.db).getStagesByRaceIds(raceRows.map(row => row.id));
        const currentDate = new GameStateRepository_1.GameStateRepository(this.db).getCurrentDate();
        return {
            program: (0, mappers_1.mapRaceProgram)(programRow),
            races: raceRows.map((row) => {
                const stages = stagesByRaceId.get(row.id) ?? [];
                return (0, mappers_1.mapRaceWithSummary)(row, stages, new RiderRepository(this.db).getUpcomingStageSummary(stages, row.is_stage_race === 1, currentDate));
            }),
        };
    }
    getRiderStats(riderId) {
      ...
      `).all(rider.activeTeamId);
            const riderAge = rider.age ?? (new GameStateRepository_1.GameStateRepository(this.db).getCurrentSeason() - rider.birthYear);
            if (riderAge <= 23) {
                const potentialMentors = teamRows.filter(m => m.id !== rider.id &&
                    m.age >= 31 &&
                    m.overall_rating >= 73 &&
            ...
            if (riderAge >= 31 && rider.overallRating >= 73) {
                mentoredRiderNames = teamRows.filter(m => m.id !== rider.id &&
                    m.age <= 23 &&
                    (rider.riderType === m.rider_type ||
                        (rider.specialization1 && m.rider_type === rider.specialization1) ||
                        (rider.specialization2 && m.rider_type === rider.specialization2) ||
                        (rider.specialization3 && m.rider_type === rider.specialization3))).map(m => `${m.first_name.charAt(0)}. ${m.last_name}`);
            }
        }
        ...
        const pointsByTerrain = (0, mappers_1.emptyRiderStatsPointsByTerrain)();
        const pointsByRaceFormat = (0, mappers_1.emptyRiderStatsPointsByRaceFormat)();
        if (!(0, mappers_1.tableExists)(this.db, 'results') || !(0, mappers_1.tableExists)(this.db, 'stages')) {
            return this.createEmptyRiderStatsPayload(rider, currentSeasonRank, currentSeasonPoints, currentSeasonRaceStats.raceDays, currentSeasonBreakawayAttempts, careerWins, careerRaceDaysBySeason, programSummary, mentorName, mentoredRiderNames);
        }
    ...
    `).all(mappers_1.RESULT_TYPE_IDS.gc, riderId);
      ...
      LEFT JOIN season_point_events final_points
       ...
       AND final_points.award_type = CASE results.result_type_id
         WHEN ${mappers_1.RESULT_TYPE_IDS.gc} THEN 'gc_final'
    ...
    `).all(riderId);
        ...
        return {
            ...
            teamName: rider.activeTeamId != null ? new TeamRepository_1.TeamRepository(this.db).getTeamById(rider.activeTeamId)?.name ?? null : null,
    ...
    getSeasonRaceStatsByRiderId(season, riderId) {
        ...
        const argsResults = riderId != null ? [mappers_1.RESULT_TYPE_IDS.stage, season, riderId] : [mappers_1.RESULT_TYPE_IDS.stage, season];
        const argsResultsGc = riderId != null ? [mappers_1.RESULT_TYPE_IDS.gc, season, riderId] : [mappers_1.RESULT_TYPE_IDS.gc, season];
        // 1. Race Days
    ...
    loadRaceFormSourcesByRiderId(riderIds, season, currentDate) {
      ...
      `).all(...riderIds, seasonStart, seasonEnd);
            for (const row of awardRows) {
                ...
                sources.push({
                    date: row.award_date,
                    amount: mappers_1.RACE_FORM_BUILD_SOURCE_AMOUNT,
    ...
    loadRaceFormSourceLabelsByDate(season) {
        ...
        if (!(0, mappers_1.tableExists)(this.db, 'stages') || !(0, mappers_1.tableExists)(this.db, 'races')) {
            return labelsByDate;
        }
        const rows = this.db.prepare(`
      SELECT
    ...
    getRiderById(id) {
      ...
      WHERE riders.id = ?
    `).get(id);
        return row ? (0, mappers_1.mapRider)(row, season, currentDate, 0) : null;
    }
    ...
    loadRiderFormHistoryByRiderId(riderIds, season) {
        ...
        if (riderIds.length === 0 || !(0, mappers_1.tableExists)(this.db, 'rider_form_history')) {
    ...
    buildFormDebugSeries(rider, history, seasonStart, seasonEnd, currentDate) {
        ...
        const points = [];
        for (let date = seasonStart; date <= seasonEnd; date = (0, mappers_1.addDaysIso)(date, 1)) {
            ...
            const projected = (0, mappers_1.resolveProjectionPoint)(date, rider.seasonFormPeakDates ?? []);
            points.push({
                date,
                sForm: projected.sForm,
                rForm: projected.rForm,
                totalForm: (0, mappers_1.roundToTwoDecimals)(projected.sForm + projected.rForm),
                isProjection: date > currentDate,
            });
        }
        return points;
    }
    createEmptyRiderStatsPayload(rider, currentSeasonRank, currentSeasonPoints, currentSeasonRaceDays, currentSeasonBreakawayAttempts, careerWins, careerRaceDaysBySeason, programSummary, mentorName, mentoredRiderNames) {
        return {
            riderId: rider.id,
            riderName: `${rider.firstName} ${rider.lastName}`,
            age: rider.age ?? (new GameStateRepository_1.GameStateRepository(this.db).getCurrentSeason() - rider.birthYear),
            ...
            currentSeasonBreakawayAttempts,
            careerWins,
            pointsByTerrain: (0, mappers_1.emptyRiderStatsPointsByTerrain)(),
            pointsByRaceFormat: (0, mappers_1.emptyRiderStatsPointsByRaceFormat)(),
            careerRaceDaysBySeason,
            seasons: [],
            careerStats: this.getRiderCareerStats(rider.id),
            fatigueHistory: [],
...
exports.RiderRepository = RiderRepository;
