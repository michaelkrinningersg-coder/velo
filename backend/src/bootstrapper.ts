...
function seedRaceProgramProbabilityRules(db: Database.Database): void {
  ...
  const programs = db.prepare('SELECT id, name FROM race_programs').all() as Array<{ id: number; name: string }>;
...
function seedStages(db: Database.Database): void {
  ...
  for (const [index, row] of rows.entries()) {
    ...
    insert.run(
      ...
      stageScore,
      row['allowed_weather'] ?? '1|2|3|4|5|6|7',
    );
...
export function seedQuickSimProfiles(db: Database.Database): void {
  ...
  for (const [index, row] of rows.entries()) {
    ...
    insert.run(
      ...
      real(req(row, 'bunched_share_intercept', ctx), `${ctx} / bunched_share_intercept`),
...
function seedWetter(db: Database.Database): void {
  ...
  for (const [index, row] of rows.entries()) {
    ...
    insert.run(
      int(req(row, 'id', ctx), ctx),
      req(row, 'wetter_name', ctx),
      real(row['effekt_sturz_min'] ?? '0', `${ctx} / effekt_sturz_min`),
      real(row['effekt_sturz_max'] ?? '0', `${ctx} / effekt_sturz_max`),
      real(row['effekt_defekt_min'] ?? '0', `${ctx} / effekt_defekt_min`),
      real(row['effekt_defekt_max'] ?? '0', `${ctx} / effekt_defekt_max`),
      real(row['windkanten_gefahr_min'] ?? '0', `${ctx} / windkanten_gefahr_min`),
      real(row['windkanten_gefahr_max'] ?? '0', `${ctx} / windkanten_gefahr_max`),
      real(row['effekt_fatigue_min'] ?? '0', `${ctx} / effekt_fatigue_min`),
      real(row['effekt_fatigue_max'] ?? '0', `${ctx} / effekt_fatigue_max`),
      real(row['breakaway_bonus_min'] ?? '0', `${ctx} / breakaway_bonus_min`),
      real(row['breakaway_bonus_max'] ?? '0', `${ctx} / breakaway_bonus_max`),
    );
  }
...
function newgenInsertColumns(prefix: string, suffix: string): string {
...
export function seedNewgenStartPresets(db: Database.Database): void {
  ...
  for (const [index, row] of rows.entries()) {
    ...
    const minValues = NEWGEN_SKILL_FIELDS.map((field) =>
      real(req(row, `min_${field}`, `${ctx} / min_${field}`), `${ctx} / min_${field}`),
    );
    ...
    insert.run(
      int(req(row, 'preset_id', ctx), ctx),
      req(row, 'type_key', ctx),
      req(row, 'display_name', ctx),
      int(req(row, 'weight', ctx), ctx),
      ...minValues,
      ...maxValues,
...
export function seedNewgenPotentialPresets(db: Database.Database): void {
  ...
  for (const [index, row] of rows.entries()) {
    ...
    const minValues = NEWGEN_SKILL_FIELDS.map((field) =>
      real(req(row, `min_pot_${field}`, `${ctx} / min_pot_${field}`), `${ctx} / min_pot_${field}`),
    );
...
function seedRiderNames(db: Database.Database): void {
