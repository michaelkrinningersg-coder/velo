...
export class RiderProgramService {
  ...
  public ensureSeasonPrograms(season: number, assignedOn?: string): void {
    ...
    `).all(season, season) as Array<{ id: number; role_name: string | null; overall_rating: number; skill_cobble: number; team_id: number }>;
    ...
    `).all(season, season, season) as Array<{ id: number; role_name: string | null; overall_rating: number; skill_cobble: number; team_id: number }>;
    ...
    const getBaseProgramName = (progId: number): string => {
    ...
    `).all() as Array<{ role_name: string; program_id: number; probability: number }>;

    const rulesByRole = new Map<string, Array<{ baseProgram: string; probability: number }>>();
    ...
    // Exclusions helper
    const isProgramExcluded = (baseProgram: string, role: string, skillCobble: number): boolean => {
    ...
    const assignedRiders: Array<{ riderId: number, teamId: number, baseProgram: string, roleName: string, overall: number }> = [];
    ...
    const groups = new Map<string, typeof assignedRiders>();
  ...
  public ensureLieutenants(season: number, assignedDate: string): void {
    ...
    const teamsWithLts = new Set(
      ...
      `).all(season, season, season).map((row: any) => row.team_id as number)
    );
    ...
    const ridersByTeam = new Map<number, typeof riders>();
    ...
    this.db.transaction(() => {
      for (const [teamId, teamRiders] of ridersByTeam.entries()) {
        ...
        const helpers = teamRiders.filter(r =>
          r.role_name === 'Edelhelfer' ||
        ...
        for (const captain of top3Captains) {
          ...
          if (specTarget == null) continue;

          let lieutenant: typeof riders[number] | null = null;
          ...
          if (lieutenant) {
            ...
            `).get(season, captain.id) as { program_id: number } | undefined;
        ...
        if (strongestSprinter) {
          ...
          if (availableForSprint.length > 0) {
            ...
            const sprinterProg = this.db.prepare(`
              SELECT program_id FROM rider_season_programs WHERE season = ? AND rider_id = ?
            `).get(season, strongestSprinter.id) as { program_id: number } | undefined;
