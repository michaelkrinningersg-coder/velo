...
function runTest() {
  ...
  const historyCheck = db.prepare('SELECT * FROM draft_history WHERE season = 2027 AND rider_id = ?').get(selectedRiderId) as any;
  if (!historyCheck) {
  ...
  const gsRow = db.prepare('SELECT draft_status FROM game_state WHERE id = 1').get() as any;
  ...
  const roleCheck = db.prepare('SELECT COUNT(*) as c FROM riders WHERE role_id IS NOT NULL AND is_retired = 0').get() as any;
  ...
  const programCheck = db.prepare('SELECT COUNT(*) as c FROM rider_season_programs').get() as any;
