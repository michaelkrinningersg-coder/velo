...
function randomizedDfs(candidates, selected, currentStages, targetStages) {
  if (currentStages === targetStages) {
    return [...selected];
  }
  const available = candidates.filter(r => !hasOverlap(selected, r));
  // Shuffle
  available.sort(() => Math.random() - 0.5);

  for (const r of available) {
    if (currentStages + r.number_of_stages <= targetStages) {
      selected.push(r);
      const res = randomizedDfs(candidates, selected, currentStages + r.number_of_stages, targetStages);
      if (res) return res;
      selected.pop();
    }
  }
...
function solveProg10() {
  ...
  for (let trial = 0; trial < 1000; trial++) {
    const wtSol = randomizedDfs(candidatesWt, initialWt, initialStages, 106);
    if (!wtSol) continue;
    
    const proSol = randomizedDfs(candidatesPro.filter(r => !hasOverlap(wtSol, r)), [], 0, 19);
    if (proSol) {
...
function solveProg11() {
  ...
  for (let trial = 0; trial < 1000; trial++) {
    const wtSol = randomizedDfs(candidatesWt, initialWt, initialWtStages, 106);
    if (!wtSol) continue;
    ...
    const proSol = randomizedDfs(candidatesPro.filter(r => !hasOverlap(wtSol, r)), initialPro, 1, 19);
    if (proSol) {
      return { wt: wtSol, pro: proSol };
    }
  }
...
function solveProg12(p10SpringIds, p11SpringIds) {
  ...
  for (let trial = 0; trial < 20000; trial++) {
    const wtSol = randomizedDfs(candidatesWt, initialWt, initialWtStages, 51);
    if (!wtSol) continue;

    const proSol = randomizedDfs(candidatesPro.filter(r => !hasOverlap(wtSol, r)), initialPro, 1, 94);
    if (!proSol) continue;

    const allSelected = [...wtSol, ...proSol];

    // A. "Vorbereitungsrennen vor der Tour im Juni"
    // Tour starts July 4. Check if there is a race in June:
