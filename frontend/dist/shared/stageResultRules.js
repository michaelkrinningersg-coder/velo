...
export const TIME_LIMIT_PERCENT_BY_PROFILE = {
    Flat: 12,
    Rolling: 14,
    Hilly: 16,
    Hilly_Difficult: 17,
...
export function roundStageResultSeconds(value) {
    return Math.max(0, Math.round(value));
}
export function isTimeTrialProfile(profile) {
    return profile === 'ITT' || profile === 'TTT';
}
export function resolveTimeLimitPercent(profile) {
    return TIME_LIMIT_PERCENT_BY_PROFILE[profile] ?? 20;
}
export function resolveStageTimeLimitSeconds(profile, stageTimesSeconds) {
...
export function rankStageResultEntries(entries, profile) {
