...
function encodeSplitModeValue(label) {
...
function scaleDistance(distanceMeter, stageDistanceMeters, width, paddingX) {
    if (stageDistanceMeters <= 0) {
        return paddingX;
    }
    return paddingX + (distanceMeter / stageDistanceMeters) * (width - paddingX * 2);
}
function interpolateElevation(summary, distanceMeter) {
...
function resolveElevationAxis(summary) {
...
function scaleElevation(elevation, axisMinElevation, axisMaxElevation, height, paddingTop, paddingBottom) {
...
function formatClimbLength(lengthKm) {
...
function resolveMarkerAccent(category, markerType) {
...
function buildProfileEvents(summary, stageDistanceMeters, width, paddingX, height, paddingTop, paddingBottom, axisMinElevation, axisMaxElevation) {
    const rawEvents = [];
    const pendingClimbs = [];
    let finishCategory = null;
...
function renderProfileEvent(event, topGuideY, baselineY) {
...
function renderCompactProfileEvent(event, topGuideY, baselineY) {
    return `
    <g class="race-sim-marker-group">
...
function buildDistanceTicks(summary, stageDistanceMeters) {
    const tickMeters = new Set();
...
function renderDistanceTicks(tickMeters, summary, stageDistanceMeters, width, paddingX, baselineY) {
...
function renderCluster(cluster, summary, stageDistanceMeters, width, height, paddingX, paddingTop, paddingBottom, axisMinElevation, axisMaxElevation, isSelected) {
...
function formatFinishGap(seconds) {
    return `+${Math.max(0, Math.round(seconds))}s`;
}
function buildSplitLabels(summary) {
    return buildIntermediateSplitLabels(summary);
}
function buildTimingEntries(snapshot, bootstrap) {
...
function renderTimingRail(summary, snapshot, bootstrap, mode) {
    ...
    const formatTimingValue = (entry) => activeSplitLabel
    ...
    const renderTimingIdentity = (entry) => {
        const flag = entry.sourceRider ? renderFlag(entry.sourceRider.country?.code3 ?? entry.sourceRider.nationality) : '';
...
function renderIttRiderLabels(clusters, summary, stageDistanceMeters, width, height, paddingX, paddingTop, paddingBottom, axisMinElevation, axisMaxElevation, bootstrap) {
    const riderById = new Map(bootstrap.riders.map((rider) => [rider.id, rider]));
    const teamAbbreviationById = new Map((bootstrap.teams ?? []).map((team) => [team.id, team.abbreviation]));
    return clusters
...
function buildClimbHighlightAreaPath(summary, stageDistanceMeters, width, paddingX, height, paddingTop, paddingBottom, axisMinElevation, axisMaxElevation, startKm, endKm) {
...
function buildStaticProfileMarkup(summary, stageProfile, label, compact, options = {}) {
    ...
    const climbHighlightPath = options.selectedClimbRange != null
        ? buildClimbHighlightAreaPath(summary, stageDistanceMeters, width, paddingX, height, paddingTop, paddingBottom, axisMinElevation, axisMaxElevation, options.selectedClimbRange.startKm, options.selectedClimbRange.endKm)
        : null;
...
export function renderStaticStageProfile(container, summary, stageProfile, label, options) {
...
export function renderMiniStageProfile(container, summary, stageProfile, label) {
    if (summary.points.length < 2) {
...
export function renderRaceProfile(container, summary, snapshot, label, bootstrap, timingMode = 'finish', selectedGroupLabel = null) {
    if (summary.points.length < 2) {
        container.innerHTML = '<div class="stage-editor-empty">Noch keine Profildaten vorhanden.</div>';
        return;
    }
