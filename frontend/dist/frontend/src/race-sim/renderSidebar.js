...
const bootstrapSidebarDataCache = new WeakMap();
const sidebarRenderCache = new WeakMap();
const leaderboardSortStateCache = new WeakMap();
...
function formatStartOffset(seconds) {
...
function getEffectiveSkillClassName(rider) {
...
function getFormClassName(value) {
    if (value > 0.5) {
...
function getSlopeClassName(value) {
    if (value > 10) {
...
function buildIntermediateSplits(bootstrap) {
...
function buildMarkerRankLookup(snapshot) {
...
function resolveSplitSortValue(rider, splitKey, stageProfile, markerRanksByKey) {
...
function buildColumns(bootstrap, splitMarkers, showSplitColumns) {
    const columns = [
...
function buildColumnsKey(columns) {
    return columns.map((column) => `${column.label}:${column.width}:${column.className ?? ''}`).join('|');
}
function buildLayoutKey(columnsKey, sortState) {
...
function getBootstrapSidebarData(bootstrap) {
...
function applyLeaderboardLayout(container, columns) {
    const scrollContainer = container.parentElement;
    const header = scrollContainer?.querySelector('.race-sim-leaderboard-head');
    if (!scrollContainer || !header) {
        return '';
    }
    const toolbar = scrollContainer.querySelector('.race-sim-leaderboard-toolbar')
        ?? (() => {
...
function updateTitle(element, value) {
...
function hasChanged(rowCache, key, value) {
    return rowCache.lastValues[key] !== value;
}
function commitValue(rowCache, key, value) {
    rowCache.lastValues[key] = value;
}
function getLeaderboardSortState(container) {
...
function renderHeaderCell(column, sortState) {
...
function resolveDefaultSortDirection(sortKey) {
    if (sortKey === 'name'
        || sortKey === 'team'
...
function buildRiderComparator(bootstrap, splitMarkers, markerRanksByKey, sortState, riderById, gcByRiderId, teamAbbreviationById) {
    if (sortState.autoSort) {
        return (left, right) => (bootstrap.stage.profile === 'ITT'
...
function canReuseOrderedRiders(previousOrderedRiderIds, ridersById, comparator) {
...
function resolveSortValue(rider, sourceRider, sortKey, bootstrap, markerRanksByKey, gcByRiderId, teamAbbreviationById) {
...
function sortRiders(riders, bootstrap, splitMarkers, markerRanksByKey, sortState, riderById, gcByRiderId, teamAbbreviationById, previousOrderedRiderIds) {
    if (!sortState.manualSortKey) {
        if (sortState.autoSort) {
        ...
        return [...riders].sort((left, right) => ((isDnfRider(left) === isDnfRider(right) ? 0 : isDnfRider(left) ? 1 : -1)
            ||
    ...
    if (canReuseOrderedRiders(previousOrderedRiderIds, ridersById, comparator)) {
        return previousOrderedRiderIds.map((riderId) => ridersById.get(riderId)).filter((rider) => rider != null);
    }
    return [...riders].sort(comparator);
}
export function handleRaceSimSidebarInteraction(container, target) {
    const teamToggleButton = target.closest('button[data-race-sim-ttt-team-toggle]');
    if (teamToggleButton) {
    ...
    const detailToggleButton = target.closest('button[data-race-sim-rider-toggle]');
    ...
    const sortState = getLeaderboardSortState(container);
    const splitsToggleButton = target.closest('button[data-race-sim-splits-toggle]');
    ...
    const autoSortButton = target.closest('button[data-race-sim-auto-sort]');
    if (autoSortButton) {
        ...
        if (sortState.autoSort) {
            ...
            sortState.manualSortKey = null;
            sortState.manualSortDirection = 'asc';
            sortState.frozenOrder = Array.from(container.querySelectorAll('[data-race-sim-rider-row]'))
                .map((row) => Number(row.dataset['raceSimRiderRow']))
                .filter((value) => Number.isFinite(value));
        }
        return true;
    }
    const sortButton = target.closest('button[data-race-sim-sort-key]');
...
function hasAnySplitTime(rider, splitMarkers) {
    return splitMarkers.some((split) => rider.splitTimes[split.key] != null);
}
function isDnfRider(rider) {
    return rider.finishStatus === 'dnf';
}
function compareIttLeaderboard(left, right, splitMarkers) {
    if (isDnfRider(left) !== isDnfRider(right)) {
        return isDnfRider(left) ? 1 : -1;
    }
    ...
    if (left.finishTimeSeconds != null)
        return -1;
    if (right.finishTimeSeconds != null)
        return 1;
...
function compareStandardLeaderboard(left, right) {
    if (isDnfRider(left) !== isDnfRider(right)) {
        return isDnfRider(left) ? 1 : -1;
    }
...
function buildEffectiveSkillLines(rider, sourceRider) {
...
function buildEffectiveSkillTitle(rider, sourceRider) {
    return buildEffectiveSkillLines(rider, sourceRider).join('\n');
}
function formatScaledMicroForm(value) {
    return formatSigned(Math.max(-2.5, Math.min(2.5, value * 2.5)));
}
function formatDraftPackInfluence(rider) {
...
function renderRiderButton(rider, isOpen) {
...
function renderTeamCell(sourceRider, teamAbbreviationById, teamNameById) {
...
function renderTeamJerseyCell(sourceRider, teamById, teamNameById) {
    if (sourceRider?.activeTeamId == null) {
        return '—';
    }
...
function renderSpecialFormDot(rider) {
...
function renderSplitCell(rider, splitKey, stageProfile, markerRanksByKey) {
    if (stageProfile !== 'ITT' && stageProfile !== 'TTT') {
...
function renderDetailPanel(rider, sourceRider, gcStanding) {
...
function buildSidebarRowCache(rider, sourceRider, isOpen, splitMarkers, teamById, teamAbbreviationById, teamNameById) {
    ...
    const nameButton = nameField.querySelector('.race-sim-row-name-btn');
    if (!nameButton) {
...
function updateSidebarRow(rowCache, position, rider, sourceRider, isDetailOpen, splitMarkers, stageProfile, markerRanksByKey, raceIsStageRace, stageNumber, gcByRiderId) {
    const seasonForm = sourceRider?.formBonus ?? 0;
    const raceForm = sourceRider?.raceFormBonus ?? 0;
...
function renderTeamTimeTrialButton(team, isOpen) {
...
function renderTeamJerseyCellByTeam(team) {
...
function buildTeamTimeTrialRows(snapshot, bootstrap, riderById) {
...
function renderTeamTimeTrialDetail(row, sidebarData, raceIsStageRace, stageNumber, openDetailRiderId) {
    return `
...
function renderTeamTimeTrialSidebar(container, snapshot, bootstrap) {
    ...
    container.innerHTML = teamRows.map((row, index) => {
      ...
      <article class="race-sim-row${index === 0 ? ' race-sim-row-leader' : ''}${isOpen ? ' race-sim-row-detail-open' : ''}">
        <div class="race-sim-row-grid">
          ...
          ${splitMarkers.map((split) => `<strong>${esc(row.splitTimes[split.key] != null ? formatClock(row.splitTimes[split.key]) : '—')}</strong>`).join('')}
...
export function renderRaceSimSidebar(container, snapshot, bootstrap) {
    ...
    const sortedRiders = sortRiders(snapshot.riders, bootstrap, splitMarkers, markerRanksByKey, sortState, sidebarData.riderById, sidebarData.gcByRiderId, sidebarData.teamAbbreviationById, previousCache?.orderedRiderIds ?? []);
    ...
    for (let index = 0; index < sortedRiders.length; index += 1) {
        ...
        if (!rowCache) {
            rowCache = buildSidebarRowCache(rider, sourceRider, cached.openDetailRiderId === rider.riderId, visibleSplitMarkers, sidebarData.teamById, sidebarData.teamAbbreviationById, sidebarData.teamNameById);
    ...
    for (let index = 0; index < sortedRiders.length; index += 1) {
        ...
        updateSidebarRow(rowCache, index + 1, rider, sourceRider, cached.openDetailRiderId === rider.riderId, visibleSplitMarkers, bootstrap.stage.profile, markerRanksByKey, bootstrap.race.isStageRace, bootstrap.stage.stageNumber, sidebarData.gcByRiderId);
