...
function compareDraftOrder(left, right) {
    ...
    if (isClassifiedFinisher(left))
        return -1;
    if (isClassifiedFinisher(right))
        return 1;
    return left.rider.id - right.rider.id;
}
...
export class SimulationEngine {
    ...
    resolveBreakawayReferenceSkill(rider, activeNonBreakawayRiders) {
    ...
    updateBreakawayGapPenaltyFromCheckpoints(activeBreakawayRiders, activeNonBreakawayRiders) {
        ...
        const bestNonBreakawayRider = [...activeNonBreakawayRiders].sort((left, right) => (right.distanceCoveredMeters - left.distanceCoveredMeters
            || right.currentSpeedMps - left.currentSpeedMps
            || left.rider.id - right.rider.id))[0] ?? null;
        ...
        while (this.nextBreakawayGapPenaltyFallbackIndex < this.breakawayGapPenaltyFallbackCheckpointsMeters.length) {
            ...
            const breakawayLeaderCrossingTime = activeBreakawayRiders
                ...
                .filter((crossingTime) => crossingTime != null)
    ...
    resolveStaminaPenalty(staminaSkill, distanceMeters) {
