...
import { calculateStageFavorites } from './stageFavorites';
...
import { ATTACK_SKILL_BONUS, COUNTER_ATTACK_DURATION_SECONDS, precalculateStageAttacks, resolveCounterAttackStarterIds, updateActiveStageAttacks, } from './stageAttacks';
...
function buildStageScoringWeightMap(rules) {
    const map = new Map();
...
export function getWeatherRelation(profileId, weatherId) {
    const profile = WEATHER_PROFILES[profileId] || WEATHER_PROFILES[1];
    if (profile.pref.includes(weatherId))
        return 'pref';
    if (profile.malus.includes(weatherId))
        return 'malus';
    return 'neutral';
}
...
function resolveDeterministicRatio(input) {
    let hash = 2166136261;
...
function resolveDraftRetentionFactor(gradientPercent) {
...
function resolveDraftDistanceCapMeters(terrain) {
    if (terrain === 'Flat') {
...
function terrainToSkillName(terrain) {
    switch (terrain) {
        case 'Flat':
            return 'Flat';
        case 'Hill':
...
function resolveBaseSkill(rider, skillName) {
    switch (skillName) {
        case 'Flat':
...
function resolveConditionFormBonus(rider) {
...
function formatSkillBreakdown(rider, components) {
...
function sampleInitialWindSpeedKph() {
...
function sampleDailyForm() {
    return roundToTwoDecimals(randomBetween(-3, 3));
}
function createWindZones(stageDistanceMeters) {
    const zones = [];
    let currentMeter = 0;
    ...
    while (currentMeter < stageDistanceMeters) {
        ...
        windSpeedKph = clamp(windSpeedKph + (Math.random() < 0.5 ? -1 : 1) * randomBetween(2, 10), 5, 70);
        vector = clamp(vector + (Math.random() < 0.5 ? -1 : 1) * randomBetween(0, 0.5), -1, 1);
    }
    return zones;
}
function compareRiders(left, right) {
    ...
    if (leftClassifiedFinisher)
        return -1;
    if (rightClassifiedFinisher)
        return 1;
...
function isRiderInactive(rider) {
    return rider.isInactiveCached ?? (rider.finishStatus === 'dnf' || rider.finishTimeSeconds != null);
}
function isClassifiedFinisher(rider) {
    return rider.isFinisherCached ?? (rider.finishStatus !== 'dnf' && rider.finishTimeSeconds != null);
}
function syncRiderStatusCache(rider) {
    rider.isInactiveCached = rider.finishStatus === 'dnf' || rider.finishTimeSeconds != null;
    rider.isFinisherCached = rider.finishStatus !== 'dnf' && rider.finishTimeSeconds != null;
}
/**
 ...
 */
function resolveSpecializationTieBreakAdjustment(entry, context, isClimberMalusStage = false, markerCategory = null) {
    ...
    if (context === 'sprint_intermediate') {
        ...
        if (markerCategory === '4') {
            if (spec1 === 'Sprint')
                ...
                adjustment -= 1.5;
            if (role === 'Edelhelfer' || role === 'Starke Helfer')
                adjustment += 2;
            if (role === 'Wassertraeger')
                adjustment += 1;
            if (role === 'Co-Kapitaen')
                adjustment -= 1;
            if (role === 'Kapitaen')
        ...
        if (spec1 === 'Attacker' || spec2 === 'Attacker')
            adjustment += 3;
    }
    return adjustment;
}
 ...
 */
function resolveMarkerCrossingTieBreakAdjustment(entry, context, riderStateById, isClimberMalusStage = false, markerCategory = null) {
...
function normalizeMarkerClassificationEntries(entries, applyTimeTieGroups, markerType = null, riderStateById = null, isClimberMalusStage = false) {
    const tieContext = markerType === 'sprint_intermediate'
        ? 'sprint_intermediate'
...
function orderRoadRiders(riders, finishContext, isClimberMalusStage) {
    ...
    let group = [];
    let previousTime = null;
    const effectiveScore = (r) => r.photoFinishScore;
    const flushGroup = () => {
...
function compareDraftOrder(left, right) {
    const leftInactive = left.isInactiveCached ?? false;
    const rightInactive = right.isInactiveCached ?? false;
    if (leftInactive !== rightInactive) {
        return leftInactive ? 1 : -1;
    }
    ...
    const leftFinisher = left.isFinisherCached ?? false;
    const rightFinisher = right.isFinisherCached ?? false;
    if (leftFinisher && rightFinisher) {
...
function resolveDraftPackFactor(groupSize) {
    const clampedGroupSize = groupSize < 1 ? 1 : (groupSize > DRAFT_PACK_HARD_CAP_SIZE ? DRAFT_PACK_HARD_CAP_SIZE : groupSize);
    return DRAFT_PACK_FACTORS[clampedGroupSize];
}
function resolveLiveWeightProfileValue(rider, weights) {
    ...
    return Object.entries(weights).reduce((sum, [skillKey, weight]) => {
        ...
        const staminaPenalty = skillKey === 'stamina'
            ...
            : 0;
        const effectiveSkill = Math.max(0, rider.rider.skills[skillKey] + formBonus + rider.dailyForm + rider.microForm + rider.teamGroupBonus - staminaPenalty);
        return sum + (effectiveSkill * weight);
    }, 0);
}
function buildLiveWeightProfileBreakdown(rider, weights) {
    const formBonus = rider.conditionFormBonus ?? 0;
    return Object.entries(weights)
        .filter((entry) => Boolean(entry[1]))
        .map(([skillKey, weight]) => {
        const resolvedSkillKey = skillKey;
        const staminaPenalty = resolvedSkillKey === 'stamina'
            ? rider.staminaSkillPenalty + rider.incidentStaminaPenalty
            : 0;
        const effectiveSkill = Math.max(0, rider.rider.skills[resolvedSkillKey] + formBonus + rider.dailyForm + rider.microForm + rider.teamGroupBonus - staminaPenalty);
        return {
...
let lastOrderedRiders = null;
let lastStartIndex = -1;
let lastEndIndex = -1;
let lastSize = 0;
let lastMaxGapMeters = 0;
function resolveDraftGroupWindow(orderedRiders, riderIndex, maxGapMeters) {
    if (lastOrderedRiders !== orderedRiders) {
        lastOrderedRiders = orderedRiders;
        lastStartIndex = -1;
        lastEndIndex = -1;
        lastSize = 0;
        lastMaxGapMeters = 0;
    }
    if (lastEndIndex >= riderIndex && Math.abs(maxGapMeters - lastMaxGapMeters) < 0.0001) {
        return {
            startIndex: lastStartIndex,
            endIndex: lastEndIndex,
            size: lastSize,
            positionInGroup: riderIndex - lastStartIndex,
        };
    }
...
function isFrontOfLargeDraftGroup(groupSize, positionInGroup) {
...
export class SimulationEngine {
    constructor(bootstrap, options) {
        ...
        this.weightedSkillComponentsByTerrain = new Map();
        this.staminaWeightByTerrain = new Map();
        this.skillBreakdownCache = new Map();
        this.teamSprintRandomValues = new Map();
        this.teamSprintSpecialRandomValues = new Map();
        this.teamBestSprinterRiderId = new Map();
        this.lastTeamGroupBonusByRiderId = null;
        this.draftOrderScratch = [];
        this.elapsedSeconds = 0;
        this.activeStageAttacksByRiderId = new Map();
        this.nextBreakawayGapPenaltyMarkerIndex = 0;
        this.nextBreakawayGapPenaltyFallbackIndex = 0;
        this.breakawayGapStatus = null;
        this.breakawayPhaseActive = false;
        this.breakawayGroupPhaseEnded = false;
        this.breakawayPhaseEnded = false;
        this.breakawayCaughtLogged = false;
        this.messages = [];
        this.allEvents = [];
        this.nextMessageId = 1;
        this.superTeamProtectedLeaderIds = new Set();
        this.superTeamBonusAmount = 0;
        this.superTeamMalusAmount = 0;
        this.superTeamStartPercent = 0;
        this.superTeamEndPercent = 0;
        this.superTeamBreakawayRiderCaught = false;
        this.hasLoggedFinishSprintTieBreak = false;
        this.hasAppliedSprintLeadoutBonuses = false;
        ...
        bootstrap.riders = bootstrap.riders.map((originalRider) => {
            ...
            const skillsToModify = ['flat', 'mountain', 'stamina', 'bikeHandling', 'recuperation', 'downhill'];
            if (relation === 'pref') {
        ...
        this.finalSpreadConfigMap = new Map();
        const terrains = ['Flat', 'Hill', 'Medium_Mountain', 'Mountain', 'High_Mountain', 'Cobble', 'Cobble_Hill', 'Abfahrt', 'Sprint'];
        ...
        const baseRiderStates = bootstrap.riders.map((rider) => {
            const riderState = {
                rider,
                ...
                baseSkillsCached: (() => {
                    const cache = {};
                    const ts = ['Flat', 'Hill', 'Medium_Mountain', 'Mountain', 'High_Mountain', 'Cobble', 'Cobble_Hill', 'Abfahrt', 'Sprint'];
                    for (const t of ts) {
        ...
        const precalculatedStageAttacks = precalculateStageAttacks(top15FavoriteRiders, bootstrap.stage, bootstrap.stageSummary, (candidate) => {
        ...
        this.breakawayPlan = precalculateStageBreakaway(bootstrap.riders, bootstrap.race, bootstrap.stage, bootstrap.stageSummary, this.stageFavorites, bootstrap.gcStandings, bootstrap.mountainStandings, bootstrap.teams);
        ...
        const riderStates = baseRiderStates.map((riderState) => {
        ...
        if (this.breakawayPlan && this.breakawayPlan.superTeamId != null) {
            ...
            const normRole = (name) => (name ?? '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
            ...
            const superTeamBreakawayRider = this.riders.find(r => r.rider.activeTeamId === this.superTeamId && this.breakawayPlan?.riderIds.includes(r.rider.id));
        ...
        for (const r of this.riders) {
            ...
            const getSkillName = (key) => SKILL_GERMAN_LABELS[key] || key;
        ...
        const effectsList = [];
        if (rolledSturz > 0)
            effectsList.push(`Sturzwahrscheinlichkeit +${rolledSturz.toFixed(1)}%`);
        if (rolledDefekt > 0)
            effectsList.push(`Defektwahrscheinlichkeit +${rolledDefekt.toFixed(1)}%`);
        if (rolledWind > 0)
            effectsList.push(`Windkanten-Gefahr +${(rolledWind * 100).toFixed(1)}%`);
        if (rolledFatigue > 0)
            effectsList.push(`Fatigue +${rolledFatigue.toFixed(1)}%`);
        if (rolledBreakaway > 0)
            effectsList.push(`Ausreißer-Bonus +${rolledBreakaway.toFixed(1)}`);
    ...
    applyPersistentStageRaceState(rider) {
    ...
    buildMarkerClassifications(ordered) {
        ...
        return this.intermediateMarkers.map((marker) => {
            ...
            const entries = normalizeMarkerClassificationEntries(markerEntries, !this.isTimeTrialMode, marker.markerType, riderStateById, this.isClimberMalusStage());
            return {
    ...
    advanceSubstep(deltaSeconds) {
        ...
        if (this.superTeamId != null && this.superTeamBreakawayRiderId != null && !this.superTeamBreakawayRiderCaught) {
            ...
            if (breakawayRiderState && !isRiderInactive(breakawayRiderState)) {
                ...
                const leaderCaughtHim = leaders.some(l => l.distanceCoveredMeters >= 0.40 * this.stageDistanceMeters &&
        ...
        for (const rider of this.riders) {
            ...
            rider.currentSegmentCached = segment;
            rider.currentWindZoneCached = windZone;
            if (!segment || !windZone) {
            ...
            this.calculateBasePhysics(rider, segment, windZone);
        ...
        if (this.isTeamTimeTrial) {
            ...
            for (let index = 0; index < ordered.length; index += 1) {
                ...
                const segment = rider.currentSegmentCached;
        ...
        if (this.breakawayPlan && this.breakawayPhaseActive && !this.breakawayCaughtLogged) {
            ...
            if (activeBreakawayRiders.length > 0 && activeNonBreakawayRiders.length > 0) {
                const lastSurvivor = activeBreakawayRiders.reduce((best, r) => r.distanceCoveredMeters > best.distanceCoveredMeters ? r : best, activeBreakawayRiders[0]);
                const leadingChaser = activeNonBreakawayRiders.reduce((best, r) => r.distanceCoveredMeters > best.distanceCoveredMeters ? r : best, activeNonBreakawayRiders[0]);
    ...
    applyTeamTimeTrialTempo(stepStartSeconds, stepEndSeconds) {
        const activeRidersByTeamId = new Map();
        for (const rider of this.riders) {
        ...
        for (const teamRiders of activeRidersByTeamId.values()) {
            ...
            const teamSpeedMps = this.resolveTimeTrialSpeedMps(teamEffectiveSkill, referenceRider.distanceCoveredMeters, segment, referenceRider.gradientModifier, referenceRider.windModifier) * resolveTttTerrainSpeedMultiplier(segment.terrain, this.bootstrap.skillWeightRules ?? []);
    ...
    resolveProjectedIttStartScore(rider) {
        let weightedScore = 0;
        let totalDistance = 0;
        for (const segment of this.bootstrap.stageSummary.segments) {
            ...
            const { effectiveSkill } = this.resolveEffectiveSkill(rider, weightedSkill.value, effectiveStaminaPenalty, 0, segmentCenterMeter);
            const gradientPercent = clamp(segment.gradient_percent, -20, 20);
    ...
    resolveStartOffsetSeconds(rider, riderIndex, teamStartIndexByTeamId) {
        if (this.isIndividualTimeTrial) {
    ...
    buildBreakawayGapPenaltyConfig() {
        ...
        const fallbackCheckpointsMeters = [];
        const firstCheckpointMeters = Math.max(BREAKAWAY_GAP_PENALTY_FALLBACK_STEP_METERS, Math.ceil(fallbackStartDistanceMeters / BREAKAWAY_GAP_PENALTY_FALLBACK_STEP_METERS) * BREAKAWAY_GAP_PENALTY_FALLBACK_STEP_METERS);
        for (let checkpointMeters = firstCheckpointMeters; checkpointMeters < breakawayPlan.groupPhaseEndDistanceMeters; checkpointMeters += BREAKAWAY_GAP_PENALTY_FALLBACK_STEP_METERS) {
            fallbackCheckpointsMeters.push(checkpointMeters);
        }
        return { markers, fallbackCheckpointsMeters };
    }
    calculateBasePhysics(rider, segment, windZone) {
        ...
        const { effectiveSkill, staminaPenalty, elevationPenalty } = this.resolveEffectiveSkill(rider, baseSkill, effectiveStaminaPenalty, teamGroupBonus, rider.distanceCoveredMeters, currentElevationMeters);
        const gradientPercent = clamp(segment.gradient_percent, -20, 20);
        ...
        rider.staminaPenalty = staminaPenalty;
        rider.elevationPenalty = elevationPenalty;
        rider.gradientPercent = gradientPercent;
        rider.gradientModifier = gradientModifier;
        rider.windModifier = windModifier;
        rider.tempSpeedMps = this.isTimeTrialMode
    ...
    resolveRoadStageSpeedMps(effectiveSkill, distanceMeters, segment, gradientModifier, windModifier, speedSkillFactor) {
    ...
    resolveElevationSkillSpreadFactor(segment, elevationMeters) {
    ...
    resolveRoadSpeedSkillFactor(terrain) {
        if (terrain === 'Flat') {
            const windkantenGefahr = this.bootstrap.stage.rolledWindkantenGefahr ?? 0;
            return 0.12 + windkantenGefahr;
        }
        if (terrain === 'Abfahrt' || terrain === 'Downhill')
            return 0.15;
        if (terrain === 'Cobble')
            return 20 / 35;
        if (terrain === 'Cobble_Hill')
            return 25 / 35;
        return 10 / 35;
    }
    resolveBreakawayReferenceSkillCached(rider, bestSkillByTerrain, overallBestSkill) {
    ...
    resolveBreakawaySkillBonus(rider) {
        if (!this.isBreakawayBonusWindow(rider)) {
            return 0;
        }
        return this.breakawayPlan?.skillBonus ?? 0;
    }
    resolveMaxBreakawayDraftModifier(rider, segment, groupSize) {
        if (groupSize <= 1) {
    ...
    resolveBreakawayTimeGapPenalty(gapSeconds) {
    ...
    recordBreakawayFallbackCheckpointCrossings(rider, startDistanceMeters, endDistanceMeters, stepStartSeconds, speedMps) {
    ...
    updateBreakawayGapPenaltyFromCheckpoints(activeBreakawayRiders, activeNonBreakawayRiders) {
        const breakawayPlan = this.breakawayPlan;
        ...
        let bestNonBreakawayRider = null;
        ...
        while (this.nextBreakawayGapPenaltyMarkerIndex < this.breakawayGapPenaltyMarkers.length) {
            ...
            let breakawayLeaderCrossing = null;
        ...
        while (this.nextBreakawayGapPenaltyFallbackIndex < this.breakawayGapPenaltyFallbackCheckpointsMeters.length) {
            ...
            let breakawayLeaderCrossingTime = null;
            for (const candidate of activeBreakawayRiders) {
    ...
    updateBreakawayGapStatus() {
        ...
        const activeBreakawayRiders = this.riders.filter((rider) => (!isRiderInactive(rider)
            && breakawayRiderIds.has(rider.rider.id)
            && rider.activeTerrain !== 'Finish'));
        const activeNonBreakawayRiders = this.riders.filter((rider) => (!isRiderInactive(rider)
            && !breakawayRiderIds.has(rider.rider.id)
            && rider.activeTerrain !== 'Finish'));
        if (activeBreakawayRiders.length === 0 || activeNonBreakawayRiders.length === 0) {
            this.breakawayGapStatus = null;
    ...
    applyBreakawaySkillTempo(deltaSeconds) {
        const breakawayPlan = this.breakawayPlan;
        ...
        for (const rider of activeBreakawayRiders) {
            ...
            rider.tempSpeedMps = this.resolveRoadStageSpeedMps(breakawayTargetSkill, rider.distanceCoveredMeters, segment, rider.gradientModifier, rider.windModifier, this.resolveRoadSpeedSkillFactor(segment.terrain));
    ...
    resolveTimeTrialSpeedMps(effectiveSkill, distanceMeters, segment, gradientModifier, windModifier) {
        return this.resolveRoadStageSpeedMps(effectiveSkill, distanceMeters, segment, gradientModifier, windModifier, 0.5);
    }
    syncRiderTelemetry(rider, teamGroupBonusByRiderId = null) {
        const segment = rider.currentSegmentCached !== undefined ? rider.currentSegmentCached : (rider.currentSegmentCached = this.currentSegment(rider));
        const windZone = rider.currentWindZoneCached !== undefined ? rider.currentWindZoneCached : (rider.currentWindZoneCached = this.currentWindZone(rider));
    ...
    updateBreakawayMalusRecovery() {
        ...
        for (const rider of this.riders) {
            ...
            const recoveredMalus = Math.max(minimumRemainingMalus, rider.breakawayInitialMalus - (recoverySteps * BREAKAWAY_MALUS_RECOVERY_STEP_SKILL));
    ...
    syncBreakawayPlanRidersTelemetry(teamGroupBonusByRiderId) {
        const breakawayPlan = this.breakawayPlan;
        if (!breakawayPlan) {
            return;
        }
    ...
    applyBreakawayGroupTempo(deltaSeconds) {
        const breakawayPlan = this.breakawayPlan;
        if (!breakawayPlan || !this.isBreakawayGroupPhaseActive() || this.isTimeTrialMode) {
            return;
        }
        ...
        const bestBreakawayRider = [...activeBreakawayRiders].sort((left, right) => (right.distanceCoveredMeters - left.distanceCoveredMeters
            || right.currentSpeedMps - left.currentSpeedMps
            || left.rider.id - right.rider.id))[0];
        if (!bestBreakawayRider) {
            return;
        }
        ...
        for (const rider of activeBreakawayRiders) {
            ...
            if (gapToBestMeters > 0) {
                ...
                rider.nextDistanceCoveredMeters = Math.min(leaderTargetDistance, rider.distanceCoveredMeters + (catchUpSpeedMps * deltaSeconds));
                continue;
            }
    ...
    resolveAttackSkillBonus(rider) {
        return rider.isAttacking ? ATTACK_SKILL_BONUS : 0;
    }
    isActiveBreakawayRider(rider) {
        return (this.breakawayPlan?.riderIds.includes(rider.rider.id) ?? false)
            && this.isBreakawayGroupPhaseActive();
    }
    isBreakawayGroupPhaseActive() {
        return this.breakawayPhaseActive
            && !this.breakawayGroupPhaseEnded
            && !this.breakawayPhaseEnded;
    }
    isBreakawayBonusWindow(rider) {
        return (this.breakawayPlan?.riderIds.includes(rider.rider.id) ?? false)
            && this.breakawayPhaseActive
            && this.breakawayGroupPhaseEnded
            && !this.breakawayPhaseEnded;
    }
    resolveAttackStagePositionExcludingBreakaways(riderId) {
    ...
    isDraftBlockedByAttack(candidate) {
    ...
    canReceiveDraftFromCandidate(rider, candidate) {
    ...
    resolvePreStageGcRank(riderId) {
        return this.bootstrap.gcStandings.find((standing) => standing.riderId === riderId)?.rank ?? null;
    }
    formatRiderWithPreStageGc(riderId, riderName) {
        const gcRank = this.resolvePreStageGcRank(riderId);
        return gcRank != null ? `${riderName} (${gcRank}.)` : riderName;
    }
    triggerStageAttacksForRider(rider, previousDistance, currentDistance, activeStepStartSeconds) {
        if (isRiderInactive(rider) || rider.isAttacking) {
            return;
        }
        ...
        if (currentStagePosition <= 0 || currentStagePosition > 10) {
            console.log(`[RaceAttack] verworfen: ${rider.riderName} Attacke ${nextAttack.attackNumber} bei ${(nextAttack.triggerDistanceMeters / 1000).toFixed(1).replace('.', ',')} km, `
        ...
        for (const counterRiderId of counterRiderIds) {
            ...
            if (!counterRider || isRiderInactive(counterRider) || counterRider.isAttacking) {
                continue;
            }
    ...
    buildTeamGroupBonusByRiderId() {
        const bonusByRiderId = new Map();
        ...
        for (let index = 0; index < activeTeamRiders.length; index += 1) {
            ...
            while (endIndex < activeTeamRiders.length
            ...
            while (startIndex < activeTeamRiders.length
    ...
    resolveTeamGroupBonusValue(rider, teamGroupBonusByRiderId) {
        if (this.isTimeTrialMode || rider.rider.activeTeamId == null) {
            return 0;
        }
        return teamGroupBonusByRiderId?.get(rider.rider.id) ?? 0;
    }
    resolveEffectiveSkill(rider, baseSkill, staminaPenalty, teamGroupBonus, distanceMeters, currentElevationMeters) {
        ...
        const baseWithForm = baseSkill
            ...
            + breakawaySkillBonus
            + teamGroupBonus
            - rider.breakawayMalus;
        const skillAfterStamina = Math.max(0, baseWithForm);
        const elevationPenalty = distanceMeters === rider.distanceCoveredMeters
            ? this.resolveRiderElevationPenalty(rider, currentElevationMeters)
            : this.resolveElevationPenalty(rider, distanceMeters);
        return {
    ...
    precalculateStaminaEndPenalty(staminaSkill) {
    ...
    resolveStaminaPenalty(rider, distanceMeters) {
    ...
    resolveRiderStaminaPenalty(rider) {
    ...
    interpolateStaminaDistanceValue(distanceKm) {
    ...
    buildSpreadCurve(lateStageStartRatio) {
    ...
    resolveSkillSpreadFactor(distanceCoveredMeters, segment) {
        ...
        const lateProgress = clamp((distanceRatio - this.lateStageStartRatio) / Math.max(0.0001, this.finalPushStartRatio - this.lateStageStartRatio), 0, 1);
        ...
        const peakProgress = clamp((distanceRatio - this.finalPushStartRatio) / Math.max(0.0001, 1 - this.finalPushStartRatio), 0, 1);
        const pushedSpread = lateMultiplier + ((peakMultiplier - lateMultiplier) * peakProgress);
        return Math.max(baseSpread, pushedSpread);
    }
    scaleFinalSpreadMultiplier(multiplier) {
        return 1 + ((Math.max(0, multiplier) - 1) * this.finalSpreadDifficultyMultiplier);
    }
    interpolateFinalSpread(targetMultiplier, progress) {
        return 1 + ((targetMultiplier - 1) * progress);
    }
    resolveWeightedSkillComponents(terrain) {
        const cached = this.weightedSkillComponentsByTerrain.get(terrain);
        if (cached) {
            return cached;
        }
        const components = resolveSkillWeightComponents(this.simulationMode, terrain, this.skillWeightRuleMap).map((component) => ({ key: component.key, weight: component.weight }));
        this.weightedSkillComponentsByTerrain.set(terrain, components);
        return components;
    }
    resolveWeightedSkillExact(rider, terrain) {
    ...
    resolveWeightedSkill(rider, terrain, staminaSkillPenalty = 0) {
        const components = this.resolveWeightedSkillComponents(terrain);
        let totalWeight = 0;
        let weightedSum = 0;
        const isCaptain = rider.role?.name === 'Kapitaen' || rider.role?.name === 'Co-Kapitaen';
        ...
        return {
            value,
            breakdown: this.isInstantSimulation ? '' : this.resolveSkillBreakdown(rider, terrain, components),
        };
    }
    resolveSkillBreakdown(rider, terrain, components) {
    ...
    resolvePrimarySkillKey(skillName) {
        switch (skillName) {
            case 'Flat':
                return 'flat';
            case 'Hill':
    ...
    resolveElevationPenaltyFromElevation(rider, elevationMeters) {
    ...
    resolveElevationPenalty(rider, distanceMeters) {
        return this.resolveElevationPenaltyFromElevation(rider, this.interpolateElevation(distanceMeters));
    }
    resolveRiderElevationPenalty(rider, currentElevationMeters) {
    ...
    resolveSegmentElevationExact(segment, distanceMeters) {
    ...
    logFinishSprintTieBreakIfNeeded() {
        ...
        console.groupCollapsed(`[FinishSprintTieBreak] ${stageLabel} | erste Zielgruppe (${orderedFirstFinishGroup.length} Fahrer) | Zeitfenster <= ${TIME_TIE_THRESHOLD_SECONDS.toFixed(2)} s`);
        console.log(`[FinishSprintTieBreak] Sortierung: erst Zielzeit, dann Score (photoFinish + SpecAdjustment) absteigend, dann Fahrer-ID.`);
        console.log(`[FinishSprintTieBreak] Kontext: ${finishContext}${climberMalusStage ? ' | Bergfahrer-Malus aktiv' : ''}`);
        orderedFirstFinishGroup.forEach((rider, index) => {
            ...
            console.log(`#${index + 1} Zielsprint | ${rider.riderName} | Zeit ${timeLabel} | Score (ohne Boni): ${skillScore.toFixed(2)} -> Score (mit Boni): ${rider.photoFinishScore.toFixed(2)} [SpecAdj: ${specAdj > 0 ? '+' : ''}${specAdj.toFixed(2)}, Leadout: +${leadoutBonus.toFixed(2)}] | ID-Tiebreak ${rider.rider.id} | (${breakdown})`);
        });
        console.groupEnd();
    }
    recordIntermediateSplits(rider, startDistanceMeters, endDistanceMeters, stepStartSeconds, speedMps) {
        if (speedMps <= 0) {
            return;
        }
        while (rider.nextIntermediateIndex < this.intermediateMarkers.length) {
            ...
            const tieContext = marker.markerType === 'sprint_intermediate'
                ? 'sprint_intermediate'
    ...
    resolveMarkerCrossingScore(rider, marker) {
    ...
    calculatePreLeadoutFinishScore(rider) {
        ...
        const baseScore = Object.entries(this.finishWeightProfile).reduce((sum, [skillKey, weight]) => {
            ...
            const effectiveSkill = Math.max(0, rider.rider.skills[skillKey] + formBonus + rider.dailyForm + rider.microForm + rider.teamGroupBonus - effectiveStaminaPenalty);
            return sum + (effectiveSkill * weight);
        }, 0);
        const specAdj = resolveSpecializationTieBreakAdjustment(rider, this.resolveFinishMarkerType(), this.isClimberMalusStage());
        return baseScore + specAdj;
    }
    applySprintLeadoutBonuses() {
        ...
        const teamSprintersMap = new Map();
        for (const rider of this.riders) {
            if (rider.finishStatus !== 'dnf' &&
                rider.finishStatus !== 'otl' &&
                rider.finishStatus !== 'dns' &&
    ...
    calculateSprintLeadoutBonusForRider(rider) {
        const teamId = rider.rider.activeTeamId;
        if (teamId == null) {
            return 0;
        }
        ...
        let bestTeammateId = null;
        const contributions = [];
        for (const r of teamRiders) {
            if (r.rider.id === rider.rider.id) {
                continue;
            }
            if (r.finishStatus === 'dnf' || r.finishStatus === 'otl' || r.finishStatus === 'dns') {
                continue;
            }
            let metCount = 0;
    ...
    resolveRiderClockSeconds(rider) {
        if (isClassifiedFinisher(rider)) {
    ...
    applyIncident(rider, incident, eventTimeSeconds, isMassCrash = false) {
        ...
        if (incident.isMassCrashTrigger && incident.massCrashPotentialRiderIds) {
            ...
            for (const victimId of incident.massCrashPotentialRiderIds) {
                ...
                if (distanceDiff <= 50) {
                    ...
                    const victimIncident = buildDynamicCrashIncident(victimState.rider, this.bootstrap.riders, incident.triggerDistanceKm, this.bootstrap.stageSummary.distanceKm);
