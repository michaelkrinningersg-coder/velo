...
export class SimulationEngine {
    constructor(bootstrap, options) {
        ...
        this.breakawayPlan = precalculateStageBreakaway(bootstrap.riders, bootstrap.race, bootstrap.stage, bootstrap.stageSummary, this.stageFavorites, bootstrap.gcStandings, bootstrap.mountainStandings);
    ...
    resolveRoadSpeedSkillFactor(skillName) {
        if (skillName === 'Flat')
            return 0.14;
        if (skillName === 'Downhill')
            return 0.18;
        return 10 / 35;
    }
    ...
    applyBreakawaySkillTempo(deltaSeconds) {
        ...
        for (const rider of activeBreakawayRiders) {
            ...
            rider.tempSpeedMps = this.resolveRoadStageSpeedMps(breakawayTargetSkill, rider.distanceCoveredMeters, segment, rider.gradientModifier, rider.windModifier, this.resolveRoadSpeedSkillFactor(rider.skillName));
    ...
    calculateSprintLeadoutBonus(rider) {
        ...
        const activeSprinters = teamRiders.filter((r) => r.finishStatus !== 'dnf' &&
            r.finishStatus !== 'otl' &&
            r.finishStatus !== 'dns' &&
