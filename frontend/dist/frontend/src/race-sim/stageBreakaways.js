...
function getTopClassificationIdsWithExclusions(standings, excludedIds, limit) {
    return new Set(standings
        .map((standing) => standing.riderId)
        .filter((id) => id != null && !excludedIds.has(id))
        .slice(0, limit));
}
...
export function precalculateStageBreakaway(riders, race, stage, stageSummary, stageFavorites, gcStandings, mountainStandings, teams) {
    ...
    let superTeamId;
    const protectedLeaderIds = new Set();
    if (isSuperTeamEligible) {
        ...
        for (const fav of fullRiderRanking) {
            ...
            if (rider.activeTeamId == null || !top10Gc.has(rider.id))
                continue;
    ...
    if (superTeamId != null) {
        ...
        if (eligibleSuperteamRiders.length > 0) {
            const sortedSupport = [...eligibleSuperteamRiders].sort((a, b) => b.skills.attack - a.skills.attack || b.overallRating - a.overallRating || b.id - a.id);
    ...
    const selectedRiders = sampleWeightedWithoutReplacement(eligibleRiders, Math.max(0, Math.min(desiredBreakawaySize - (forcedRider ? 1 : 0), eligibleRiders.length)), (rider) => weightByRiderId.get(rider.id)?.finalWeight ?? 1);
    if (forcedRider) {
