import { api } from '../api';
import { $, esc, state, renderTeamNameLink, formatDate, formatRaceTime, renderFlag, renderMiniJersey, findRiderById, showModal, resolveRaceCategoryBadgeStyle, buildRaceCategoryBadgeCssVariables, formatNonFinisherReason, renderSeasonFormPhaseIndicator, } from '../state';
import { formatRaceDateRange, renderStageProfileBadge, raceCategoryNameBadge, openDashboardStageProfile } from './dashboard';
import { renderStageEditorScoreBadge } from './stageEditor';
let comparedRiders = [];
let selectedCompareTeamId = null;
let chartToggles = {
    form: true,
...
export function renderWeatherIcon(weatherId, weatherName) {
    if (weatherId == null)
        return '';
 ...
 */
export function renderRiderStatsRaceBadge(categoryName, isStageRace, numberOfStages) {
...
export function renderRiderStatsCategoryBadge(categoryName) {
    if (!categoryName)
        return '-';
...
export function renderRiderRankBadge(rank) {
    if (rank == null)
        return '-';
...
export function renderFilterButton(label, isActive, activeBg, activeText, hoverBorderColor, dataAttr, dataValue) {
...
export function resolveRiderStatsFinalTypeClassName(rowType) {
    switch (rowType) {
        case 'gc_final':
            return 'is-gc';
        case 'points_final':
...
export function getRiderStatsRowTypeLabel(rowType) {
    switch (rowType) {
        case 'gc_final':
...
export function renderRiderStatsFinalTypeBadge(rowType) {
...
export function renderProfileWinBadge(value, profileType, label) {
...
export function renderWeatherWinBadge(value, weatherId, label) {
    let style = 'padding: 0.2rem 0.6rem; border-radius: 20px; font-size: 0.8rem; font-weight: bold; min-width: 1.8rem; text-align: center; display: inline-block; box-sizing: border-box;';
    if (value === 0) {
        style += 'background: rgba(255, 255, 255, 0.05); color: rgba(255, 255, 255, 0.2); border: 1px solid rgba(255, 255, 255, 0.1);';
    }
...
export function formatRiderStatsRaceBlockMeta(block) {
...
export function resolveCurrentSeasonRank(riderId) {
...
export function getRiderStatsFinalTypeSortOrder(rowType) {
    switch (rowType) {
        case 'gc_final':
            return 0;
        case 'points_final':
...
export function sortRiderStatsRowsNewestFirst(rows) {
...
export function sortRiderStatsRaceBlocksNewestFirst(blocks) {
    return [...blocks]
        .map((block) => ({
        ...block,
...
export function formatRiderStatsCareerRaceDaysTooltip(payload) {
...
export function formatRiderStatsAvailabilityTitle(isUnavailable, healthStatusLabel, unavailableUntil, unavailableDaysRemaining) {
    if (!isUnavailable) {
        return null;
    }
...
export function renderRiderStatsAvailabilityMarker(rider, payload) {
...
export function renderRiderOverallRatingBadge(score) {
    const stops = [
...
export function renderRiderStatsIconHeatPill(icon, title, value, maxValue) {
...
export function getSpecializationIcon(specialization) {
    if (!specialization)
        return '';
...
export function renderRiderStatsSummary(rider, payload, teamName, countryCode, countryFlag) {
...
export function renderRiderStatsTitle(rider, payload) {
    const firstName = payload?.firstName ?? rider?.firstName ?? '';
    const lastName = payload?.lastName ?? rider?.lastName ?? '';
    return `${esc(firstName)} <strong>${esc(lastName)}</strong>`;
}
export function renderRiderStatsHeatPill(label, value, maxValue) {
    const ratio = maxValue > 0 ? Math.max(0, Math.min(1, value / maxValue)) : 0.5;
    const hue = Math.round(6 + (ratio * 118));
    const borderAlpha = 0.26 + (ratio * 0.18);
    const bgAlpha = 0.14 + (ratio * 0.12);
...
export function renderRiderStatsTerrainSources(payload) {
...
export function renderRiderStatsRaceFormatSources(payload) {
...
export function renderRiderStatsTabs(payload) {
...
export function renderRiderSkillBadge(score) {
    const stops = [
...
export function renderRiderStatsSkillsTab(rider, payload) {
    ...
    for (let i = 0; i < 6; i++) {
        ...
        const skillVal = riderSkills[skillsToDraw[i]] ?? minVal;
    ...
    skillsToDraw.forEach((skillKey, i) => {
        const val = riderSkills[skillKey] ?? minVal;
    ...
    const sortedSkills = [...skillsList].sort((a, b) => {
        const sa = riderSkills[a.key] ?? 60;
        const sb = riderSkills[b.key] ?? 60;
        return sb - sa;
    });
    ...
    sortedSkills.forEach((skill, idx) => {
        const score = riderSkills[skill.key] ?? 60;
        const html = `
...
export function renderRiderStatsFatigueTab(rider, payload) {
...
export function renderRiderStatsFormTab(payload) {
    if (!payload) {
        return `
...
export function renderRiderStatsProgramTab(payload) {
...
export function renderStatusDotsColumn(row) {
    const activeJerseys = new Set();
    if (row.jerseysWorn) {
        const keys = row.jerseysWorn.split(',').map((k) => k.trim()).filter(Boolean);
...
export function renderRiderStatsRankBadge(label, variant) {
    return `<span class="rider-stats-rank-badge rider-stats-rank-badge-${variant}">${esc(label)}</span>`;
}
export function renderRiderStatsBreakaway(row) {
...
export function renderRiderStatsPlacement(row) {
...
export function renderRiderStatsBody(rider, payload, isLoading = false) {
        ...
        ${seasonToRender.raceBlocks.map((block) => `
          <section class="rider-stats-race-block">
            <div class="rider-stats-race-head">
              ...
              ${renderRiderStatsRaceBadge(block.raceCategoryName, block.isStageRace, block.rows.filter((r) => r.rowType === 'stage_result').length || null)}
            </div>
            <div class="dashboard-race-stages-table-wrap rider-stats-table-wrap">
              <table class="data-table rider-stats-table">
                <colgroup>
...
function updateRiderStatsModalWidth() {
    const card = document.querySelector('.rider-stats-modal-card');
    if (!card)
        return;
...
function getRoleBadge(roleName, roleId) {
...
export async function openRiderStats(riderId) {
...
export function initRiderStatsListeners() {
    $('rider-stats-body').addEventListener('click', (event) => {
        const rankBtn = event.target.closest('button[data-top-results-rank]');
        if (rankBtn) {
        ...
        const filterBtn = event.target.closest('button[data-top-results-filter]');
        if (filterBtn) {
            const filterType = filterBtn.dataset['topResultsFilter'];
        ...
        if (event.target && event.target.id && event.target.id.startsWith('toggle-chart-')) {
        ...
        if (!tabButton) {
            // Handle remove click
            const removeBtn = event.target.closest('button[data-remove-compare-id]');
            if (removeBtn) {
            ...
            const pageButton = event.target.closest('button[data-top-results-page]');
            if (pageButton) {
            ...
            const profileLinkButton = event.target.closest('button[data-stage-profile-id]');
...
export function renderRiderStatsTopResultsTab(payload) {
    const allRows = [];
    ...
    const categories = Array.from(new Set(allRows.map(r => r.raceCategoryName).filter(Boolean)));
    ...
    let filteredRows = allRows.filter(r => {
        ...
        if (isFinalRow) {
            if (r.rowType === 'gc_final')
                return state.riderStatsTopResultsFilters.gc;
            if (r.rowType === 'mountain_final')
                return state.riderStatsTopResultsFilters.mountain;
            if (r.rowType === 'points_final')
                return state.riderStatsTopResultsFilters.points;
            if (r.rowType === 'youth_final')
                return state.riderStatsTopResultsFilters.youth;
            if (r.rowType === 'breakaway_final')
                return state.riderStatsTopResultsFilters.breakaway;
            return true;
        }
        else {
            if (r.isStageRace) {
...
export function renderRiderStatsContractsTab(payload) {
...
export function renderRiderStatsCareerTab(payload) {
    ...
    const careerRaceDays = (payload.careerRaceDaysBySeason || []).reduce((sum, r) => {
        const days = r.raceDays ?? r.race_days ?? r.racedays ?? 0;
        return sum + Number(days);
    }, 0);
    // Helper function to render badge
    const renderCareerBadge = (displayValue, type, title, value) => {
...
window.openRiderStatsFromRiderStats = openRiderStats;
