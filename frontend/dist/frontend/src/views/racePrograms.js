import { api } from '../api';
import { $, esc, state, formatDate, } from '../state';
import { resolveRaceCategoryBadgeStyle } from '../riderStatsUi';
let activePopupRaceId = null;
let activeRiderCountPopupRaceId = null;
let programRolesSortKey = 'id';
let programRolesSortAsc = true;
// Global filter states
let filterCobble = 'all';
let filterRaceCategories = new Set();
let popoverFilterCobble = 'all';
...
function getLetterAt(name, pos) {
    const prefix = name.split('_')[0] || '';
    return prefix.charAt(pos - 1) || '';
}
function isProgramVisible(name, filterVal) {
...
function getWeekStatus(program, W) {
    const isPeak = (W >= program.peak1_min && W <= program.peak1_max) ||
        (W >= program.peak2_min && W <= program.peak2_max) ||
        (W >= program.peak3_min && W <= program.peak3_max);
    if (isPeak)
        return 'peak';
    const checkPrep = (peak_min) => {
...
function getCellColorClass(status) {
    if (status === 'peak')
        return 'cell-peak';
    if (status === 'prep')
        return 'cell-prep';
...
function isRaceInActivePhase(program, race) {
    if (race.cachedKws) {
...
function isWeekInPeakOr6WeeksAnstieg(program, W) {
    const isPeak = (W >= program.peak1_min && W <= program.peak1_max) ||
        (W >= program.peak2_min && W <= program.peak2_max) ||
        (W >= program.peak3_min && W <= program.peak3_max);
    if (isPeak)
        return true;
    const checkAnstieg = (peak_min) => {
...
function isRaceInPeakOr6WeeksAnstieg(program, race) {
    if (race.cachedKws) {
        for (const kw of race.cachedKws) {
...
function getProgramDaysPerPhase(program, assignedRaceIds, payload) {
    const peaks = [
    ...
    let phase3 = 0;
    let phase4 = 0;
    const assignedRaces = payload.races.filter((r) => assignedRaceIds.has(r.id));
    for (const race of assignedRaces) {
        if (race.cachedDays) {
            race.cachedDays.forEach((d) => {
                const kw = d.kw;
                if (kw <= peaks[0].max) {
                    phase1++;
                }
...
function checkCanAssignProgram(program, race, payload) {
    let assignedRaceIds;
    if (payload.programToRacesMap) {
        assignedRaceIds = new Set(payload.programToRacesMap.get(program.id) || []);
        assignedRaceIds.delete(race.id);
    }
    else {
        assignedRaceIds = new Set(payload.raceProgramRaces
            .filter((m) => m.program_id === program.id && m.race_id !== race.id)
            .map((m) => m.race_id));
    }
    ...
    for (const phase of touchedPhases) {
        if (phase === 'phase1') {
            if (isStageRace && daysPerPhase.phase1 > 35)
                return false;
            if (!isStageRace && daysPerPhase.phase1 > 36)
                return false;
        }
        else if (phase === 'phase2') {
            if (isStageRace && daysPerPhase.phase2 > 35)
                return false;
            if (!isStageRace && daysPerPhase.phase2 > 36)
                return false;
        }
        else if (phase === 'phase3') {
            if (isStageRace && daysPerPhase.phase3 > 35)
                return false;
            if (!isStageRace && daysPerPhase.phase3 > 36)
                return false;
        }
    }
    return true;
}
function toggleProgramRaceDirect(programId, raceId) {
    const payload = state.raceProgramsPayload;
    if (!payload)
        return;
    const idx = payload.raceProgramRaces.findIndex((m) => m.program_id === programId && m.race_id === raceId);
    ...
    if (idx === -1) {
        for (const rId of raceIdsToToggle) {
            const exists = payload.raceProgramRaces.some((m) => m.program_id === programId && m.race_id === rId);
            if (exists)
                continue;
            const targetRace = payload.races.find((r) => r.id === rId);
            if (targetRace) {
                const start = targetRace.start_date;
                const end = targetRace.end_date;
                const toRemove = [];
                payload.raceProgramRaces.forEach((m, index) => {
                    if (m.program_id === programId && m.race_id !== rId) {
                        const otherRace = payload.races.find((r) => r.id === m.race_id);
                        if (otherRace) {
        ...
        for (const rId of raceIdsToToggle) {
            const matchIdx = payload.raceProgramRaces.findIndex((m) => m.program_id === programId && m.race_id === rId);
            if (matchIdx !== -1) {
...
function enrichPayloadWithCachedData(payload) {
...
function rebuildAssignmentIndexes(payload) {
    if (!payload)
        return;
    const assignmentMap = new Map();
    const programToRacesMap = new Map();
    payload.raceProgramRaces.forEach((m) => {
        assignmentMap.set(`${m.program_id}_${m.race_id}`, m);
        if (!programToRacesMap.has(m.program_id)) {
            programToRacesMap.set(m.program_id, new Set());
        }
...
export async function loadRaceProgramsData(force = false) {
    if (state.raceProgramsPayload && !force) {
        ...
        if (filterRaceCategories.size === 0 && state.raceProgramsPayload.raceCategories) {
            filterRaceCategories = new Set(state.raceProgramsPayload.raceCategories.map((c) => c.id));
        }
        renderRacePrograms();
        return;
    }
...
export function initRaceProgramsView() {
    // Navigation tabs handler
    $('view-race-programs').addEventListener('click', (event) => {
        const target = event.target;
        // Tab buttons
        const tabBtn = target.closest('.results-type-btn[data-tab]');
        if (tabBtn) {
        ...
        const actionBtn = target.closest('.race-programs-action-btn');
        if (actionBtn) {
        ...
        const expandBtn = target.closest('.race-row-expand-btn');
        if (expandBtn) {
        ...
        const progExpandBtn = target.closest('.program-row-expand-btn');
        if (progExpandBtn) {
        ...
        const sortHeader = target.closest('.program-roles-sort-header');
        if (sortHeader) {
        ...
        const originTrigger = target.closest('.combo-origin-trigger');
        if (originTrigger) {
        ...
        const racePopoverBtn = target.closest('.race-popover-trigger');
        if (racePopoverBtn) {
        ...
        const riderCountPopoverBtn = target.closest('.race-rider-count-trigger');
        ...
        const progToggle = target.closest('.popover-program-toggle');
        if (progToggle) {
            event.stopPropagation();
    ...
    $('view-race-programs').addEventListener('click', (event) => {
        const cell = event.target.closest('.toggleable-race-cell[data-day]');
        if (!cell)
            return;
    ...
    $('view-race-programs').addEventListener('change', (event) => {
        ...
        if (target.classList.contains('filter-cobble-radio')) {
            filterCobble = target.value;
            renderRacePrograms();
            return;
        }
        if (target.classList.contains('popover-filter-cobble-radio')) {
            popoverFilterCobble = target.value;
            renderRacePrograms();
            return;
        }
...
function toggleProgramRaceAssignment(programId, dayStr) {
    const payload = state.raceProgramsPayload;
    if (!payload)
        return;
    const races = payload.races.filter((r) => r.start_date <= dayStr && r.end_date >= dayStr);
    if (races.length === 0)
        return;
    const currentMappingIndex = payload.raceProgramRaces.findIndex((m) => m.program_id === programId && races.some((r) => r.id === m.race_id));
    if (currentMappingIndex === -1) {
        ...
        for (const rId of raceIdsToAssign) {
            ...
            if (exists)
                continue;
            const raceObj = payload.races.find((r) => r.id === rId);
            if (raceObj) {
        ...
        const idx = races.findIndex((r) => r.id === currentMapping.race_id);
...
async function saveRaceProgramsData() {
    if (!state.raceProgramsPayload || state.raceProgramsSaving)
        return;
...
function calculateProgramDays(program, payload) {
    let peak = 0;
    let prep = 0;
    let none = 0;
    ...
    let assignedRaceIds;
    if (payload.programToRacesMap) {
        assignedRaceIds = payload.programToRacesMap.get(program.id) || new Set();
    }
    else {
        assignedRaceIds = new Set(payload.raceProgramRaces.filter((m) => m.program_id === program.id).map((m) => m.race_id));
    }
    const assignedRaces = payload.races.filter((r) => assignedRaceIds.has(r.id));
...
// RENDERING
export function renderRacePrograms() {
    ...
    function renderFilterCard(payload) {
        const categoryCheckboxes = payload.raceCategories.map((cat) => {
...
function renderTabCalendarCols(payload) {
    ...
    const programs = allPrograms.filter((p) => {
        const dist = distributionMap ? distributionMap.get(p.id) : programDistribution.find((row) => row.program_id === p.id);
  ...
  `;
    for (const prog of programs) {
        const stats = programStats.find((s) => s.id === prog.id)?.stats;
        const dist = distributionMap ? distributionMap.get(prog.id) : programDistribution.find((row) => row.program_id === prog.id);
        const riderCount = dist ? parseInt(dist.deterministic_rider_count || '0', 10) : 0;
        headerRow += `
    ...
    for (const day of DAYS_OF_2026) {
        const racesToday = (racesByDate ? (racesByDate.get(day.dateStr) || []) : races.filter((r) => r.start_date <= day.dateStr && r.end_date >= day.dateStr))
            .filter((r) => filterRaceCategories.has(r.category_id));
    ...
    `;
        for (const prog of programs) {
            ...
            if (assignmentMap) {
                ...
                assignedMapping = raceProgramRaces.find((m) => m.program_id === prog.id && racesToday.some((r) => r.id === m.race_id));
            }
            let content = '';
            let tdClass = `toggleable-race-cell ${colorClass}`;
            let tdAttrs = `data-day="${day.dateStr}" data-program-id="${prog.id}"`;
            if (assignedMapping) {
                const race = races.find((r) => r.id === assignedMapping.race_id);
...
function renderTabCalendarRows(payload) {
    const allPrograms = payload.programs;
    const raceProgramRaces = payload.raceProgramRaces;
    const races = payload.races;
    const programDistribution = payload.programDistribution;
    ...
    const raceAssignmentsCountMap = new Map();
    raceProgramRaces.forEach((m) => {
...
// 3. Tab: Peak Editor
function renderTabPeakEditor(payload) {
...
function getStagesSummaryForRace(raceId, stages) {
    const raceStages = stages.filter((s) => s.race_id === raceId).sort((a, b) => a.stage_number - b.stage_number);
    ...
    const stagesListHtml = raceStages.map((s) => `
...
// 4. Tab: Rider-Role
function renderTabRiderRole(payload) {
    const races = [...payload.races].sort((a, b) => a.start_date.localeCompare(b.start_date));
    const raceProgramRaces = payload.raceProgramRaces;
    ...
    const raceToProgramsMap = new Map();
    raceProgramRaces.forEach((m) => {
        if (!raceToProgramsMap.has(m.race_id)) {
            raceToProgramsMap.set(m.race_id, new Set());
        }
        raceToProgramsMap.get(m.race_id).add(m.program_id);
    });
    const rows = races.map((race) => {
        // 1. Find assigned programs
        const assignedProgramIds = raceToProgramsMap.get(race.id) || new Set();
        const assignedProgramDistributions = programDistribution.filter((row) => assignedProgramIds.has(row.program_id));
        let targetLetter = null;
        if (race.is_stage_race === 0) {
            const singleStage = stages.find((s) => s.race_id === race.id);
        ...
        const programItems = payload.programs.map((p) => {
            const isAssigned = assignmentMap ? assignmentMap.has(`${p.id}_${race.id}`) : raceProgramRaces.some((m) => m.program_id === p.id && m.race_id === race.id);
            const dist = distributionMap ? distributionMap.get(p.id) : programDistribution.find((row) => row.program_id === p.id);
            ...
            const rolesDesc = [];
            if (capt > 0)
                rolesDesc.push(`${capt} Kapitän`);
            if (coCapt > 0)
                rolesDesc.push(`${coCapt} Co-Kapitän`);
            if (elite > 0)
                rolesDesc.push(`${elite} Edelhelfer`);
            if (strong > 0)
                rolesDesc.push(`${strong} Starke Helfer`);
            if (water > 0)
                rolesDesc.push(`${water} Wasserträger`);
            if (sprint > 0)
                rolesDesc.push(`${sprint} Sprinter`);
        ...
        if (targetLetter !== null) {
            ...
            const isFlatOrRolling = (race.is_stage_race === 0) && (stages.find((s) => s.race_id === race.id)?.profile?.toLowerCase() === 'flat' || stages.find((s) => s.race_id === race.id)?.profile?.toLowerCase() === 'rolling');
            programItems.sort((a, b) => {
                ...
                if (isFlatOrRolling) {
                    ...
                    if (getLetterAt(a.program.name, 1) === targetL)
                        bucketA = 0;
                    else if (getLetterAt(a.program.name, 2) === targetL)
                        bucketA = 1;
                    else if (getLetterAt(a.program.name, 3) === targetL)
                        bucketA = 2;
                    if (getLetterAt(b.program.name, 1) === targetL)
                        bucketB = 0;
                    else if (getLetterAt(b.program.name, 2) === targetL)
                        bucketB = 1;
                    else if (getLetterAt(b.program.name, 3) === targetL)
                        bucketB = 2;
                }
                if (bucketA !== bucketB) {
        ...
        const filteredProgramItems = programItems.filter((item) => {
            if (item.count === 0)
                return false;
            return isProgramVisible(item.program.name, popoverFilterCobble);
        });
        const programItemsHtml = filteredProgramItems.map((item) => {
            ...
            let conflictHtml = '';
            if (!item.isAssigned) {
                const otherAssignments = raceProgramRaces.filter((m) => m.program_id === p.id && m.race_id !== race.id);
                const overlappingRaces = otherAssignments
                    .map((m) => payload.races.find((r) => r.id === m.race_id))
                    .filter((r) => r && r.start_date <= race.end_date && r.end_date >= race.start_date);
                if (overlappingRaces.length > 0) {
                    const raceNames = overlappingRaces.map((r) => r.name).join(', ');
      ...
      `;
        ...
        if (race.is_stage_race === 1) {
            const raceStages = stages.filter((s) => s.race_id === race.id);
            flatRollingStagesCount = raceStages.filter((s) => {
        ...
        if (race.is_stage_race === 0) {
            ...
            if (isCobbleRace) {
                ...
                const roleSpecCombosFiltered = (payload.roleSpecCombinations || []).filter((c) => assignedProgramIds.has(c.program_id));
                cobbleRidersCount = roleSpecCombosFiltered
                    .filter((c) => c.spec1 === 'Cobble' || c.spec2 === 'Cobble' || c.spec3 === 'Cobble')
    ...
    `;
      ...
      `;
        ...
        const translateSpecs = (spec1, spec2) => {
        ...
        for (const combo of sortedCombos) {
            if (combo.count > 0) {
                ...
                const origins = roleSpecCombosFiltered.filter((c) => {
                    return c.role === combo.role && c.spec1 === combo.spec1 && (c.spec2 || '—') === combo.spec2;
                });
                const originDetails = origins.map((c) => {
                    const prog = payload.programs.find((p) => p.id === c.program_id);
...
function getProgramRolesSortIndicator(key) {
    if (programRolesSortKey !== key)
...
function renderSortHeader(key, label, extraStyle = '') {
...
function renderSortHeaderLeft(key, label, extraStyle = '') {
    const activeClass = programRolesSortKey === key ? 'sort-active' : '';
    return `
    <th style="cursor: pointer; user-select: none; ${extraStyle}" class="program-roles-sort-header ${activeClass}" data-sort-key="${key}">
...
function renderTabProgramRoles(payload) {
    const programs = payload.programs;
    ...
    const combosByProgramMap = new Map();
    roleSpecCombinations.forEach((c) => {
    ...
    const programDataList = programs.map((prog) => {
    ...
    const rows = programDataList.map((item) => {
        ...
        const assignedRaceIds = programToRacesMap ? (programToRacesMap.get(prog.id) || new Set()) : new Set(payload.raceProgramRaces.filter((m) => m.program_id === prog.id).map((m) => m.race_id));
