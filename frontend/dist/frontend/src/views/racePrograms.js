...
function getProgramRegion(name) {
    const parts = name.split('_');
    if (parts.length === 3) {
        return parts[1];
    }
...
function isProgramRegionVisible(name) {
    const region = getProgramRegion(name);
    if (region === null)
        return true; // global/standard programs are always shown
...
function checkCanAssignProgram(program, race, payload) {
    const assignedRaceIds = new Set(payload.raceProgramRaces
...
function toggleProgramRaceDirect(programId, raceId) {
    ...
    if (idx === -1) {
        const targetRace = payload.races.find((r) => r.id === raceId);
        if (targetRace) {
...
function calculateProgramDays(program, payload) {
    ...
    const assignedRaceIds = new Set(payload.raceProgramRaces.filter((m) => m.program_id === program.id).map((m) => m.race_id));
    const assignedRaces = payload.races.filter((r) => assignedRaceIds.has(r.id));
...
function renderTabCalendarCols(payload) {
    ...
    const programs = allPrograms.filter((p) => {
        const dist = programDistribution.find((row) => row.program_id === p.id);
  ...
  `;
    for (const prog of programs) {
        ...
        const dist = programDistribution.find((row) => row.program_id === prog.id);
        const riderCount = dist ? parseInt(dist.deterministic_rider_count || '0', 10) : 0;
        headerRow += `
    ...
    for (const day of DAYS_OF_2026) {
        const racesToday = races.filter((r) => r.start_date <= day.dateStr && r.end_date >= day.dateStr);
    ...
    `;
        for (const prog of programs) {
            ...
            const assignedMapping = raceProgramRaces.find((m) => m.program_id === prog.id && racesToday.some((r) => r.id === m.race_id));
            let content = '';
      ...
      <div style="font-size: 0.65rem; color: var(--text-500);">W${day.weekNum}</div>
        ...
        const getRaceBadgeHtml = (index) => {
            ...
            if (!race)
                return '';
            const style = resolveRaceCategoryBadgeStyle(race.category?.name);
            const assignedCount = raceProgramRaces.filter((m) => m.race_id === race.id).length;
            return `
...
function renderTabRiderRole(payload) {
    ...
    const rows = races.map((race) => {
        ...
        const assignedProgramIds = new Set(raceProgramRaces.filter((m) => m.race_id === race.id).map((m) => m.program_id));
        ...
        const programItems = payload.programs.map((p) => {
            const isAssigned = raceProgramRaces.some((m) => m.program_id === p.id && m.race_id === race.id);
            const dist = programDistribution.find((row) => row.program_id === p.id);
        ...
        const filteredProgramItems = programItems.filter((item) => {
            ...
            if (variant >= 1 && variant <= 3 && !popoverShowV1_3)
                return false;
            if (variant >= 4 && variant <= 6 && !popoverShowV4_6)
                return false;
...
function renderTabProgramRoles(payload) {
    ...
    const programDataList = programs.map((prog) => {
        const progCombos = roleSpecCombinations.filter((c) => c.program_id === prog.id);
        let totalRiders = 0;
        const roleCounts = {
            Kapitaen: 0,
            Co_Kapitaen: 0,
    ...
    const rows = programDataList.map((item) => {
        ...
        const assignedRaceIds = new Set(payload.raceProgramRaces.filter((m) => m.program_id === prog.id).map((m) => m.race_id));
