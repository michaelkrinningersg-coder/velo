import { api } from '../api';
import { $, esc, state, isActiveView, showModal, hideModal, showLoading, hideLoading, showError, hideError, showInstantProgress, updateInstantProgress, renderFlag, raceSimView, setRaceSimView, setRealtimeCompletionInFlight, realtimeCompletionInFlight, setInstantStageInFlightId, instantStageInFlightId, setRealtimeStageLoadInFlightId, realtimeStageLoadInFlightId, activateView, } from '../state';
...
export function formatPendingStageLabel(raceName, stageNumber, profile) {
    return `${raceName} · Etappe ${stageNumber} · ${profile}`;
}
export function buildRealtimeCommitEntries(snapshot, bootstrap) {
...
export function buildRealtimeLeadoutContributions(snapshot, bootstrap) {
    const contributions = [];
...
export async function openInstantStage(stageId, skipViewActivation = false) {
...
export function getRosterEditorSelectedCount(teamId) {
...
export function isRosterEditorSelectionValid() {
...
function loadRaceSimViewInstance() {
...
export async function openRosterEditor(stageId) {
...
export async function applyRosterEditor() {
    const payload = state.rosterEditor;
    if (!payload) {
        return;
    }
...
export function renderRealtimeRaceView() {
...
export async function openRealtimeStage(stageId, activateLiveView) {
...
export async function completeRealtimeStage(stageId, entries, markerClassifications, incidents, events, skipViewActivation = false, leadoutContributions, superTeamId) {
...
export function initLiveRaceListeners() {
