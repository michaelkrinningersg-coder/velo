import { api } from '../api';
import { $, esc, state, formatDate, formatKm, formatElevationGain, isActiveView, showModal, showLoading, hideLoading, renderFlag, renderMiniJersey, findRaceById, findStageById, formatRiderName, getRiderCountryCode, getRiderRoleName, renderSkillValue, renderSeasonFormPhase, resolveRaceCategoryBadgeStyle, buildRaceCategoryBadgeCssVariables, getRiderSpecializationLabel, autoProgressActive, setAutoProgressActive, } from '../state';
...
export function raceCategoryBadge(race) {
...
export function raceCategoryNameBadge(race) {
    const categoryStyle = resolveRaceCategoryBadgeStyle(race.category?.name);
    const badgeStyle = buildRaceCategoryBadgeCssVariables(categoryStyle);
...
export function getRaceStageDateRange(race) {
...
export function canEditPendingStage(stage) {
...
async function logParticipantCountsOnce() {
    const season = state.gameState?.season;
    if (!season || state.races.length === 0)
        return;
    ...
    const results = await Promise.all(upcomingRaces.map(async (race) => {
...
export async function logProgramAssignmentsOnce() {
    const season = state.gameState?.season;
    if (!season)
        return;
    ...
    if (assignmentsByProgram.size === 0)
        return;
...
export function renderDashboardRaces() {
...
export function summarizeStageProfiles(stages) {
    if (stages.length === 0) {
...
export function getStageProfileClassName(profile) {
    return `stage-profile-badge-${profile.toLowerCase().replace(/_/g, '-')}`;
}
export function renderStageProfileBadge(profile) {
    return `<span class="stage-profile-badge ${getStageProfileClassName(profile)}">${esc(profile)}</span>`;
}
export function buildDashboardStageProfileLabel(race, stage) {
    return `${race.name} · ${getStageDisplayName(stage)} · ${stage.profile}`;
}
export async function ensureStageSummaryLoaded(stageId) {
...
export function renderDashboardRaceStagesModal() {
...
export async function openDashboardRaceStages(raceId) {
...
export function renderProgramRaceRows(payload) {
...
export async function openRiderProgram(riderId) {
...
export function renderRaceParticipantRows(participants) {
...
export function renderRaceParticipantsHeader(label, sortKey, title) {
...
export function sortRaceParticipants(participants) {
...
export function getSpecLabel(rider) {
    if (rider.specialization1 != null) {
        return getRiderSpecializationLabel(rider.specialization1);
    }
    return '–';
}
export async function openRaceProgramParticipants(raceId) {
    const race = findRaceById(raceId);
...
export async function refreshRaceProgramParticipants(showLoadingState = false) {
...
export async function openDashboardStageProfile(stageId, selectedClimb = null) {
...
export async function executeDayAdvance() {
...
export function updateAutoProgressUI() {
    const btn = document.getElementById('btn-auto-progress');
    if (!btn)
        return;
...
export function startAutoProgress() {
    if (autoProgressActive)
        return;
...
export function stopAutoProgress() {
    if (!autoProgressActive)
        return;
    setAutoProgressActive(false);
    updateAutoProgressUI();
}
async function runAutoProgressLoop() {
