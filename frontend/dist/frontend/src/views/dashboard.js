...
function renderRaceRowHtml(race) {
...
export function renderDashboardRaces() {
    ...
    const inProgressRaces = state.races.filter(race => race.startDate <= currentDate && race.endDate >= currentDate);
    const upcomingRaces = state.races.filter(race => race.startDate > currentDate && race.startDate <= maxDateStr);
    let html = '';
    // In Progress
    html += `
