import { api } from '../api';
import { $, esc, state, formatDate, formatRaceTime, formatRaceGap, renderMiniJersey, renderResultsJerseyColumn, renderResultsFlagColumn, resolveRiderCountryCode, findRiderById, findRaceById, findStageById, renderResultsParticipant, renderRiderNameLink, renderNonFinisherStatusBadge, formatNonFinisherReason, renderRankDelta, formatMarkerLabel, FLAG_CODE_BY_CODE3, GC_RESULT_TYPE_ID, POINTS_RESULT_TYPE_ID, MOUNTAIN_RESULT_TYPE_ID, } from '../state';
...
export function resolveMarkerResultsSortPriority(classification) {
...
export function sortStageMarkerClassifications(classifications) {
...
export function resolveMarkerResultButtonLabel(classification) {
...
export async function loadStageResults(stageId, silentIfMissing) {
...
export async function loadRaceRoster(raceId) {
...
function normalizeSpecPair(spec1, spec2) {
    const s1 = spec1 ?? '';
    const s2 = spec2 ?? '';
    const parts = [s1, s2].filter(Boolean).sort();
    return parts.join('+');
}
const SPEC_PAIR_COLORS = {
...
function getRoleOrder(roleId) {
    if (roleId == null)
        return 99;
    const order = { 1: 0, 2: 1, 3: 2, 4: 3, 5: 4, 6: 5 };
...
function renderRaceRoster() {
    ...
    function getRosterSpecializationLabel(spec) {
        switch (spec) {
            case 'Berg':
                return 'Berg';
            case 'Hill':
    ...
    const getTeamPoints = (teamId) => {
        if (teamId == null || !state.seasonStandings?.teamStandings)
            return 0;
    ...
    for (const team of teams) {
        team.riders.sort((a, b) => getRoleOrder(a.roleId) - getRoleOrder(b.roleId) ||
...
function renderLeadoutPopover(row) {
    ...
    const activeRaceRiderIds = new Set(state.resultsRoster && state.resultsRoster.raceId === state.selectedResultsRaceId
        ? state.resultsRoster.entries.filter(e => !e.hasDropped).map(e => e.riderId)
        : []);
    const teammates = state.riders.filter(r => r.activeTeamId === row.teamId &&
...
function getKmZeroEventPriority(row) {
...
function renderLeaderDots(riderId) {
    if (riderId == null || !state.stageResults)
        return '';
    const currentRace = findRaceById(state.selectedResultsRaceId);
    const isStageRace = currentRace?.isStageRace ?? false;
...
function formatEventTextWithAllRiders(text) {
    if (!text)
        return '';
    let tempText = text;
    const placeholders = [];
...
export function renderResultsView() {
    ...
    const selectedClassification = visibleClassifications.find((classification) => classification.resultTypeId === state.selectedResultTypeId) ?? visibleClassifications[0] ?? null;
    ...
    const stagePointsMap = new Map();
    const stageMountainPointsMap = new Map();
    if (state.stageResults) {
...
export function initResultsListeners() {
    ...
    $('results-type-tabs').addEventListener('click', (event) => {
        ...
        const specialButton = event.target.closest('button[data-results-special-view]');
        if (specialButton) {
    ...
    $('results-marker-tabs').addEventListener('click', (event) => {
        ...
        const filterButton = event.target.closest('button[data-event-filter]');
        if (filterButton) {
