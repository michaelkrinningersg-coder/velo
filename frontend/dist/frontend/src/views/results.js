...
function renderRaceRoster() {
    ...
    const getBestGCRank = (teamRiders) => {
    ...
    const getBestSeasonStandingPoints = (teamRiders) => {
        if (!state.seasonStandings?.riderStandings)
            return 0;
        let maxPts = 0;
        for (const r of teamRiders) {
...
function renderLeadoutPopover(row) {
    ...
    const activeRaceRiderIds = new Set(stageClassification
        ? stageClassification.rows.map(r => r.riderId).filter((id) => id != null)
        : []);
...
export function renderResultsView() {
    ...
    const stageBreakawayKmsMap = new Map();
    if (state.stageResults) {
