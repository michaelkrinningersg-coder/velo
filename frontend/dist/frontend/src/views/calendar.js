import { $, esc, state, formatDate, renderFlag, isActiveView, } from '../state';
import { raceCategoryBadge, raceCategoryNameBadge, openDashboardRaceStages, startAutoProgress, } from './dashboard';
...
function getCalendarWeeks(year, monthIndex) {
    const weeks = [];
...
export function initCalendarView() {
    ...
    $('calendar-weeks').addEventListener('click', (event) => {
        const bar = event.target.closest('.calendar-event-bar');
        if (bar) {
    ...
    $('calendar-races-tbody').addEventListener('click', (event) => {
        const raceBtn = event.target.closest('.dashboard-race-link');
        if (raceBtn) {
    ...
    if (container) {
        container.addEventListener('mouseenter', (event) => {
            const target = event.target.closest('[data-race-id]');
            if (target) {
...
export function showCalendarView() {
    if (state.gameState?.currentDate) {
        const [y, m] = state.gameState.currentDate.split('-').map(Number);
        currentYear = y;
        currentMonthIndex = m - 1;
...
export function renderCalendarRaceList() {
    const searchInput = $('calendar-race-search');
