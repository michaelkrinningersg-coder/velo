...
export function renderDraftView() {
    ...
    const toolbar = document.querySelector('#view-draft .results-toolbar');
...
function renderDraftCandidateBox(c, isSelected, currentTeamId, isPlayerTurn = false) {
...
function createDraftOverlayElement(season) {
  ...
  `;
    overlay.addEventListener('click', (event) => {
        ...
        const candidateBox = target.closest('.draft-candidate-clickable');
        if (candidateBox) {
            const riderId = Number(candidateBox.dataset.riderId);
            state.selectedDraftRiderId = riderId;
...
export async function startDraftPresentation(season) {
    ...
    state.draftRevealShown = false;
    state.selectedDraftRiderId = null;
...
async function renderActivePlayerTurn() {
    clearDraftTimeouts();
    const season = state.draftSelectedSeason || state.currentSave?.currentSeason || 2026;
    const displayWrap = document.getElementById('draft-overlay-pick-display');
    if (displayWrap) {
    ...
    const roundTitle = document.getElementById('draft-overlay-round-title');
    if (roundTitle)
        roundTitle.textContent = `Runde ${draftState.currentRound}`;
    const teamSubtitle = document.getElementById('draft-overlay-team-subtitle');
    if (teamSubtitle) {
    ...
    if (candList) {
        if (draftState.isPlayerTeam && draftState.candidates && draftState.candidates.length > 0) {
            candList.innerHTML = draftState.candidates.map((c) => {
                const isSelected = c.riderId === state.selectedDraftRiderId;
    ...
    if (displayWrap) {
        if (draftState.isPlayerTeam) {
            const selectedId = state.selectedDraftRiderId;
            const selectedRider = draftState.candidates?.find((c) => c.riderId === selectedId);
            if (selectedRider) {
...
function renderDraftPlayerDecisionBox(rider) {
    const season = state.draftSelectedSeason ?? state.currentSave?.currentSeason ?? 2026;
    const age = season - rider.birthYear;
    const specs = [];
    if (rider.specialization1)
        specs.push(formatSpecName(rider.specialization1));
    if (rider.specialization2)
        specs.push(formatSpecName(rider.specialization2));
    if (rider.specialization3)
        specs.push(formatSpecName(rider.specialization3));
...
async function submitPlayerDraftPick(riderId) {
    const season = state.draftSelectedSeason || state.currentSave?.currentSeason || 2026;
    ...
    try {
        ...
        alert('Fehler beim Übermitteln des Picks: ' + e.message);
    }
    finally {
        hideLoading();
    }
}
async function quickCompleteDraftFlow() {
    const season = state.draftSelectedSeason || state.currentSave?.currentSeason || 2026;
    ...
    try {
        ...
        alert('Fehler beim Beenden des Drafts: ' + e.message);
    }
    finally {
        hideLoading();
    }
