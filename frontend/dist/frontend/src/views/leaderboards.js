import { api } from '../api';
import { $, esc, renderFlag, renderMiniJersey, isActiveView, } from '../state';
import { openRiderStats } from './riderStats';
import { openTeamStats } from './teamStats';
let activeScope = 'riders';
let activePeriod = 'season';
let savedUserPeriod = 'season'; // to restore period when unlocking
let activeMetricKey = '';
...
export function initLeaderboardsView() {
    ...
    window.openRiderStatsFromLeaderboard = openRiderStats;
    window.openTeamStatsFromLeaderboard = openTeamStats;
...
export function showLeaderboardsView() {
...
function selectMetric(metricKey, sourceSelectId) {
...
function isAllTimeOnlyMetric(key) {
    return [
        'mentors_ranking',
    ].includes(key) || key.startsWith('youngest_winners');
}
function isPhysisMetric(key) {
...
function updatePeriodTabsUI() {
    ...
    if (!periodTabs)
        return;
    const seasonBtn = periodTabs.querySelector('button[data-period="season"]');
    const alltimeBtn = periodTabs.querySelector('button[data-period="alltime"]');
    if (activePeriod === 'live') {
        // Hide period tabs
...
export async function renderLeaderboard() {
    ...
    if (!emptyEl || !tableEl || !theadEl || !tbodyEl)
        return;
    ...
    if (activeScope === 'riders') {
        const wtChecked = $('leaderboard-filter-wt')?.checked ?? true;
        const ptChecked = $('leaderboard-filter-pt')?.checked ?? true;
        const otherChecked = $('leaderboard-filter-other')?.checked ?? false;
