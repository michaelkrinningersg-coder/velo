import { api } from '../api';
import { $, esc, state, renderFlag, activateView, showLoading, hideLoading, downloadTextFile, } from '../state';
import { getStageDisplayName, buildDashboardStageProfileLabel, renderStageProfileBadge, openDashboardStageProfile, } from './dashboard';
// Constants
export const STAGE_PROFILE_OPTIONS = [
    'Flat',
    'Rolling',
    'Hilly',
    'Hilly_Difficult',
...
export const STAGE_TERRAIN_OPTIONS = ['Flat', 'Hill', 'Medium_Mountain', 'Mountain', 'High_Mountain', 'Cobble', 'Cobble_Hill', 'Abfahrt', 'Sprint'];
export const STAGE_MARKER_TYPES = ['start', 'climb_start', 'climb_top', 'sprint_intermediate', 'finish_flat', 'finish_TT', 'finish_hill', 'finish_mountain'];
export const STAGE_MARKER_CATEGORIES = ['Sprint', '4', '3', '2', '1', 'HC'];
...
// Type Helpers
export function isFinishMarkerType(markerType) {
    return ['finish_flat', 'finish_TT', 'finish_hill', 'finish_mountain'].includes(markerType);
}
export function isMountainFinishMarkerType(markerType) {
    return markerType === 'finish_hill' || markerType === 'finish_mountain';
}
export function hasClimbMarkerCategory(category) {
    return category != null && ['HC', '1', '2', '3', '4'].includes(category);
}
export function isMountainClassificationMarkerType(markerType, markerCategory) {
    return markerType === 'climb_top' || (isMountainFinishMarkerType(markerType) && hasClimbMarkerCategory(markerCategory));
}
// Helpers
export function roundStageEditorOneDecimal(value) {
    return Math.round(value * 10) / 10;
}
export function roundStageEditorKm(value) {
...
export function markerCategoryOptionsHtml(selected) {
...
export function markerTypeSortValue(markerType) {
    return STAGE_MARKER_TYPES.indexOf(markerType);
}
export function sortStageMarkers(markers) {
    return [...markers].sort((left, right) => markerTypeSortValue(left.type) - markerTypeSortValue(right.type));
}
export function buildStageEditorWaypointsFromSegments(segments) {
...
export function stageEditorFieldErrorClass(hasError) {
    return hasError ? ' stage-editor-input-invalid' : '';
}
export function getStageEditorSegmentIssuesAt(draft, segmentIndex) {
...
export function collectStageEditorClimbPairIssues(draft) {
...
export function normalizeMarkerForType(marker) {
    if (marker.type === 'climb_top') {
...
export function normalizeStageEditorDraft(draft) {
...
export function applyAutomaticStageEditorTerrain(draft) {
...
export function syncStageEditorDerivedState(draft) {
...
export function recalculateStageEditorSegmentStartElevations(draft) {
    if (draft.segments.length === 0) {
        draft.waypoints = [];
        return;
    }
...
export function syncStageEditorDraftStats(draft) {
...
export function ensureStageEditorBoundaryMarkers(draft) {
...
export function renderSegmentMarkerRows(markers, segmentIndex, totalSegments, scope) {
    if (markers.length === 0) {
        return '';
    }
...
export function renderSegmentMarkerBlock(markers, segmentIndex, totalSegments, scope) {
...
export function describeMarkerScope(scope, segmentIndex, totalSegments) {
    if (scope === 'start') {
...
export function updateStageEditorMarker(segmentIndex, markerIndex, scope, field, rawValue) {
...
export function addStageEditorMarker(segmentIndex, scope) {
    if (!state.stageEditorDraft)
        return;
    const segment = state.stageEditorDraft.segments[segmentIndex];
    if (!segment)
...
export function removeStageEditorMarker(segmentIndex, markerIndex, scope) {
    if (!state.stageEditorDraft)
        return;
    const segment = state.stageEditorDraft.segments[segmentIndex];
    if (!segment)
...
let prevStageId = 0;
let prevRaceId = 0;
export async function initializeStageEditorForm() {
...
export function renderProgramsDropdown() {
    const container = $('stage-editor-programs-list');
    if (!state.stageEditorPrograms)
        return;
...
export function updateSelectedProgramsText() {
    const checked = document.querySelectorAll('input[name="stage-editor-program-selection"]:checked');
...
export function getStageEditorSegmentStartKm(draft, index) {
    let kmMark = 0;
...
export function getStageEditorSegmentEndElevation(segment) {
    return Math.round(segment.startElevation + ((segment.lengthKm * 1000) * (segment.gradientPercent / 100)));
}
export function resolveFirstFreePositiveInteger(values) {
...
export function resolveNextFreeStageEditorStageId() {
    const ids = state.stageEditorExistingStages.map((stage) => stage.stageId);
    return ids.length > 0 ? Math.max(...ids) + 1 : 1;
}
export function resolveNextFreeStageEditorRaceId() {
    const ids = [
...
export function findNextFreeStageId(startId, direction) {
...
export function findNextFreeRaceId(startId, direction) {
    let candidate = startId;
...
export function updateRaceDatesFromStageDate() {
    const stageDateInput = $('stage-editor-date');
    if (!stageDateInput)
        return;
    ...
    const newRaceCb = document.getElementById('stage-editor-new-race-checkbox');
    ...
    const startDateInput = $('stage-editor-race-start-date');
    const endDateInput = $('stage-editor-race-end-date');
...
export function setStageEditorDefaults(draft) {
    ...
    $('stage-editor-stage-id').value = String(nextStageId);
    $('stage-editor-race-id').value = String(nextRaceId);
    ...
    const checkboxes = document.querySelectorAll('input[name="stage-editor-weather"]');
    checkboxes.forEach((cb) => {
        cb.checked = true;
    });
    updateRaceDatesFromStageDate();
}
export function setStageEditorMetadataFields(metadata) {
...
export function getStageEditorIssues(draft) {
    if (!draft)
...
export function getStageEditorMetadataErrors() {
    const errors = [];
    ...
    const isNewRace = $('stage-editor-new-race-checkbox').checked;
    ...
    if (isNewRace) {
        ...
        const raceName = $('stage-editor-race-name').value.trim();
        const countryId = Number($('stage-editor-race-country').value);
        const categoryId = Number($('stage-editor-race-category').value);
        const numStages = Number($('stage-editor-race-num-stages').value);
        const startDate = $('stage-editor-race-start-date').value.trim();
        const endDate = $('stage-editor-race-end-date').value.trim();
        const prestige = Number($('stage-editor-race-prestige').value);
        if (!raceName)
            errors.push('Rennname fehlt.');
        if (!Number.isInteger(countryId) || countryId <= 0)
            errors.push('Land fehlt oder ist ungültig.');
        if (!Number.isInteger(categoryId) || categoryId <= 0)
            errors.push('Kategorie fehlt oder ist ungültig.');
        if (!Number.isInteger(numStages) || numStages <= 0)
            errors.push('Etappenanzahl muss eine positive Ganzzahl sein.');
        if (!/^\d{4}-\d{2}-\d{2}$/.test(startDate))
            errors.push('Startdatum muss im Format YYYY-MM-DD vorliegen.');
        if (!/^\d{4}-\d{2}-\d{2}$/.test(endDate))
            errors.push('Enddatum muss im Format YYYY-MM-DD vorliegen.');
        if (!Number.isInteger(prestige) || prestige < 1 || prestige > 100)
            errors.push('Prestige muss zwischen 1 und 100 liegen.');
    }
    else {
        if (!existingRaceIds.includes(raceId)) {
    ...
    const updatePrograms = $('stage-editor-program-checkbox').checked;
    if (updatePrograms) {
...
export function readStageEditorMetadata() {
    const weatherInputs = document.querySelectorAll('input[name="stage-editor-weather"]:checked');
...
export function renderClimbScoreBadge(score) {
...
export function renderStageEditorCategoryBadge(category) {
    if (category == null)
        return '<span class="stage-editor-category-empty">—</span>';
...
export function renderStageEditorProfileOpenButton(content, stageId, title, climbId) {
...
export function renderStageEditorStageScorePopover(row) {
    const fmtKm = (v) => v != null ? `${v.toFixed(1).replace('.', ',')} km` : '—';
    const fmtM = (v) => v != null ? `${Math.round(v).toLocaleString('de-DE')} m` : '—';
...
export function renderStageEditorClimbScorePopover(row) {
    const fmtKm = (v) => v != null ? `${v.toFixed(1).replace('.', ',')} km` : '—';
    const fmtM = (v) => v != null ? `${Math.round(v).toLocaleString('de-DE')} m` : '—';
...
export function renderStageEditorScoreControl(score, minScore, maxScore, stageId, popoverContent, title, climbId, customBadgeHtml) {
...
export function renderStageEditorOverviewHeader(label, sortKey, activeKey, direction, table) {
...
export function renderStageEditorStagesOverview() {
    ...
    <tr>
      ...
      <td><strong>${esc(getStageDisplayName({ stageNumber: row.stageNumber }))}</strong></td>
      <td>${renderStageEditorScoreControl(row.profileScore, 0, 100, row.stageId, renderStageEditorStageScorePopover(row), buildDashboardStageProfileLabel({ name: row.raceName }, { stageNumber: row.stageNumber, profile: row.profile }))}</td>
...
export function renderStageEditorClimbsOverview() {
    ...
    <tr>
      ...
      <td>${renderStageEditorScoreControl(row.climbScore, 0, 350, row.stageId, renderStageEditorClimbScorePopover(row), buildDashboardStageProfileLabel({ name: row.raceName }, { stageNumber: row.stageNumber, profile: 'Mountain' }), row.id, renderClimbScoreBadge(row.climbScore))}</td>
      <td>${renderFlag(row.countryCode || '')}</td>
      <td><strong>${esc(row.raceName)}</strong></td>
      <td><strong>${esc(getStageDisplayName({ stageNumber: row.stageNumber }))}</strong></td>
...
export async function loadStageEditorOverview(force = false) {
...
export async function loadStageEditorExistingStages(force = false) {
...
export function sortStageEditorStageRows(rows) {
...
export function sortStageEditorClimbRows(rows) {
...
export function getPairedClimbs(draft) {
    const climbs = [];
    const openClimbs = [];
    let currentKm = 0;
    draft.segments.forEach((segment, segmentIndex) => {
...
export function getIntermediateSprints(draft) {
    const sprints = [];
    let currentKm = 0;
...
export function getSegmentsInClimbsIndices(draft) {
...
export function shouldHideStageEditorSegment(draft, index, climbIndices) {
    const segment = draft.segments[index];
    if (!segment)
        return false;
...
export function renderStageEditor() {
    ...
    const profileSelectEl = document.getElementById('stage-editor-profile');
    ...
    const hideBoringCheckbox = $('stage-editor-hide-boring-segments-checkbox');
...
export function renderStageEditorAutomaticClimbs(draft) {
...
export function updateStageEditorWaypoint(index, field, rawValue) {
    const draft = state.stageEditorDraft;
    if (!draft)
        return;
    const segment = draft.segments[index];
...
export function insertStageEditorWaypoint(index) {
    const draft = state.stageEditorDraft;
    if (!draft)
        return;
...
export function appendStageEditorWaypoint() {
    const draft = state.stageEditorDraft;
    if (!draft)
        return;
    const lastSegment = draft.segments[draft.segments.length - 1];
...
export function deleteStageEditorWaypoint(index) {
    const draft = state.stageEditorDraft;
    if (!draft)
        return;
...
export async function onStageEditorImport() {
...
export async function onStageEditorLoadExisting() {
...
export async function onStageEditorExport() {
    if (!state.stageEditorDraft) {
    ...
    const newRace = $('stage-editor-new-race-checkbox').checked;
    const updatePrograms = $('stage-editor-program-checkbox').checked;
    let raceDetails = undefined;
    if (newRace) {
        raceDetails = {
            name: $('stage-editor-race-name').value.trim(),
            countryId: Number($('stage-editor-race-country').value),
            categoryId: Number($('stage-editor-race-category').value),
            isStageRace: Number($('stage-editor-race-is-stage-race').value) === 1,
            numberOfStages: Number($('stage-editor-race-num-stages').value),
            startDate: $('stage-editor-race-start-date').value.trim(),
            endDate: $('stage-editor-race-end-date').value.trim(),
            prestige: Number($('stage-editor-race-prestige').value),
        };
    }
    ...
    try {
        ...
        const stageNumInput = $('stage-editor-stage-number');
        ...
        $('stage-editor-stage-id').value = String(nextStageId);
        prevStageId = nextStageId;
        const newRaceCheckbox = $('stage-editor-new-race-checkbox');
        if (newRaceCheckbox)
            newRaceCheckbox.checked = false;
        const newRaceDetails = $('stage-editor-new-race-details');
        if (newRaceDetails) {
            newRaceDetails.classList.add('hidden');
            newRaceDetails.style.display = 'none';
        }
        const programCheckbox = $('stage-editor-program-checkbox');
        if (programCheckbox)
        ...
        const raceIdVal = Number($('stage-editor-race-id').value);
...
export function initStageEditorListeners() {
    ...
    const newRaceCb = $('stage-editor-new-race-checkbox');
    const newRaceDetails = $('stage-editor-new-race-details');
    const programCb = $('stage-editor-program-checkbox');
    const programDetails = $('stage-editor-program-details');
    if (newRaceCb) {
    ...
    if (startDateInput) {
        startDateInput.addEventListener('change', () => {
            ...
            if (/^\d{4}-\d{2}-\d{2}$/.test(startDate)) {
                const isStageRaceSel = document.getElementById('stage-editor-race-is-stage-race');
                ...
                if (endDateInput) {
                    if (!isStageRace) {
                        ...
                        const numStagesIn = document.getElementById('stage-editor-race-num-stages');
    ...
    const stageIdInput = $('stage-editor-stage-id');
    const raceIdInput = $('stage-editor-race-id');
    if (stageIdInput && raceIdInput) {
        ...
        const handleIdInput = (input, type) => {
