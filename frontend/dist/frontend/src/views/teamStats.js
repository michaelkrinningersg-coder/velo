import { api } from '../api';
import { $, esc, state, renderFlag, resolveTeamJerseyAssetPath, showModal, renderRiderNameLink, FLAG_CODE_BY_CODE3, } from '../state';
import { renderStageProfileBadge } from './dashboard';
import { RIDER_STATS_ICONS, renderRiderStatsRaceBadge, renderRiderStatsCategoryBadge, resolveCurrentSeasonRank, renderRiderStatsRankBadge, renderProfileWinBadge, renderWeatherWinBadge, renderStatusDotsColumn, resolveRiderStatsFinalTypeClassName, getRiderStatsRowTypeLabel, renderFilterButton } from './riderStats';
    ...
    let filteredRows = payload.topResults.filter(r => {
        ...
        if (isFinalRow) {
            ...
            if (r.rowType === 'breakaway_final')
                return state.teamStatsTopResultsFilters.breakaway;
            return true;
        }
        else {
...
export function initTeamStatsListeners() {
    ...
    $('team-stats-body').addEventListener('click', (event) => {
        ...
        // Rank filter buttons
        const rankBtn = target.closest('button[data-team-top-results-rank]');
        if (rankBtn) {
        ...
        const filterBtn = target.closest('button[data-team-top-results-filter]');
        if (filterBtn) {
            const filterType = filterBtn.dataset['teamTopResultsFilter'];
