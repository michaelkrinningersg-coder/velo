import { api } from '../api';
import { $, esc, state, renderFlag, renderMiniJersey, showModal, renderRiderNameLink, FLAG_CODE_BY_CODE3, } from '../state';
import { renderStageProfileBadge } from './dashboard';
import { RIDER_STATS_ICONS, renderRiderStatsRaceBadge, renderRiderStatsCategoryBadge, resolveCurrentSeasonRank, renderRiderStatsRankBadge } from './riderStats';
...
function getTopRidersBySpecialization(riders) {
    ...
    const gc = [...riders].map(r => ({ rider: r, score: getGCScore(r) })).sort((a, b) => b.score - a.score).slice(0, 5);
    const sprinter = [...riders].map(r => ({ rider: r, score: getSprintScore(r) })).sort((a, b) => b.score - a.score).slice(0, 5);
    const climber = [...riders].map(r => ({ rider: r, score: r.skills.mountain })).sort((a, b) => b.score - a.score).slice(0, 5);
    const puncheur = [...riders].map(r => ({ rider: r, score: r.skills.hill })).sort((a, b) => b.score - a.score).slice(0, 5);
    const cobbler = [...riders].map(r => ({ rider: r, score: r.skills.cobble })).sort((a, b) => b.score - a.score).slice(0, 5);
    const attacker = [...riders].map(r => ({ rider: r, score: r.skills.attack })).sort((a, b) => b.score - a.score).slice(0, 5);
    return {
        'Gesamtklassement': gc,
        'Sprinter': sprinter,
        'Bergfahrer': climber,
    ...
    <div style="background: var(--bg-secondary); border: 1px solid var(--border-primary); padding: 1rem; border-radius: 8px; margin-bottom: 1.5rem;">
      ...
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem; border-top: 1px solid var(--border-primary); padding-top: 0.75rem;">
        <div style="background: rgba(251, 191, 36, 0.02); border: 1px solid rgba(251, 191, 36, 0.08); padding: 0.5rem 0.75rem; border-radius: 6px;">
          <h4 style="margin: 0 0 0.5rem 0; font-size: 0.85rem; font-weight: bold; color: #fbbf24;">Die 5 formstärksten Fahrer</h4>
        ...
        <div style="background: rgba(59, 130, 246, 0.02); border: 1px solid rgba(59, 130, 246, 0.08); padding: 0.5rem 0.75rem; border-radius: 6px;">
          <h4 style="margin: 0 0 0.5rem 0; font-size: 0.85rem; font-weight: bold; color: #3b82f6;">Die 5 besten Fahrer (UCI Weltrangliste)</h4>
...
export function renderTeamStatsTopResultsTab(payload) {
    ...
    let filteredRows = payload.topResults.filter(r => {
        ...
        if (isFinalRow) {
            ...
            if (r.profile === 'TTT' || r.isStageRace || r.stageNumber != null) { // Stage race stage result
...
export function renderTeamStatsCareerTab(payload) {
    ...
    <div class="rider-stats-top-results-filters" style="display: flex; gap: 1rem; margin-bottom: 1.5rem; align-items: center; background: rgba(255, 255, 255, 0.03); padding: 0.6rem 1rem; border-radius: 8px; border: 1px solid rgba(255, 255, 255, 0.05); width: fit-content;">
      <label style="margin-right: 0.5rem; font-weight: 600; color: #ccc;">Auswahlzeitraum:</label>
      <select id="team-stats-success-season-select" class="form-control" style="width: auto; background: #222; color: #fff; border-color: #444;">
        <option value="all" ${state.teamStatsSelectedSeason === 'all' ? 'selected' : ''}>Ewig (Karrieregesamt)</option>
...
export async function openTeamStats(teamId) {
    ...
    $('team-stats-jersey').innerHTML = renderMiniJersey(teamId, team?.name ?? '');
    $('team-stats-meta').textContent = team ? `${team.abbreviation} · ${team.divisionName || team.division || '–'}` : 'Daten werden geladen';
