import { api } from './api';
import { buildRaceCategoryBadgeCssVariables, renderRiderNameLink, resolveRaceCategoryBadgeStyle, } from './riderStatsUi';
...
const STAGE_TERRAIN_OPTIONS = ['Flat', 'Hill', 'Medium_Mountain', 'Mountain', 'High_Mountain', 'Cobble', 'Cobble_Hill', 'Abfahrt', 'Sprint'];
...
const STAGE_MARKER_CATEGORIES = ['Sprint', '4', '3', '2', '1', 'HC'];
...
function hasClimbMarkerCategory(category) {
    return category != null && ['HC', '1', '2', '3', '4'].includes(category);
}
function isMountainClassificationMarkerType(markerType, markerCategory) {
...
function renderMiniJersey(teamId, teamName) {
    if (teamId == null) {
...
function renderResultsJerseyColumn(teamId, teamName) {
    return `<span class="results-jersey-cell">${renderMiniJersey(teamId, teamName)}</span>`;
}
function renderResultsFlagColumn(countryCode) {
    if (!countryCode) {
...
function resolveRiderCountryCode(riderId) {
    if (riderId == null) {
        return null;
    }
...
function renderResultsParticipant(name, strong = true, isBreakaway = false, riderId = null, teamId = null) {
...
function renderGcRankDelta(previousRank, delta) {
    if (previousRank == null || delta == null || delta === 0) {
        return '<span class="results-gc-delta results-gc-delta-neutral">â—</span>';
    }
    if (delta > 0) {
...
function resolveGcRankDelta(row, previousGcRanks) {
    if (row.riderId == null) {
...
function formatMarkerLabel(markerType, label) {
...
function resolveMarkerResultsSortPriority(classification) {
    if (isMountainClassificationMarkerType(classification.markerType, classification.markerCategory))
        return 0;
    if (classification.markerType === 'sprint_intermediate')
        return 1;
    return 2;
}
function sortStageMarkerClassifications(classifications) {
    return [...classifications].sort((left, right) => (resolveMarkerResultsSortPriority(left) - resolveMarkerResultsSortPriority(right)
...
function renderMarkerClassificationsHtml(classifications) {
...
function renderSingleMarkerClassificationHtml(classification) {
    const categoryText = classification.markerCategory ? ` · Kat. ${classification.markerCategory}` : '';
    const rows = classification.entries.map((entry) => {
        const rider = state.riders.find((candidate) => candidate.id === entry.riderId) ?? null;
        const riderName = rider ? `${rider.firstName} ${rider.lastName}` : `Fahrer ${entry.riderId}`;
...
function resolveMarkerResultButtonLabel(classification) {
...
function formatAverageSpeed(distanceKm, timeSeconds) {
    if (!(distanceKm > 0) || !(timeSeconds > 0)) {
        return '';
    }
    return `${((distanceKm / timeSeconds) * 3600).toFixed(1).replace('.', ',')} km/h`;
}
function formatPointsGap(points) {
    if (points === 0)
        return '–';
    return `-${points}`;
}
function renderSeasonCountryTopRiders(topRiders) {
...
function renderSeasonCountryNameCell(row) {
    return `
...
function renderSeasonTeamTopRiders(teamRow, riderStandings) {
...
function renderSeasonTeamNameCell(row, riderStandings) {
    return `
    <div class="season-standings-country-anchor" tabindex="0">
...
function raceCategoryBadge(race) {
...
function getRaceStageDateRange(race) {
...
function formatRaceDateRange(race) {
...
const TEAM_DETAIL_PAGE_COLUMNS = {
    skills: [
...
const TEAM_DETAIL_PAGE_LABELS = {
    skills: 'Skills',
    form: 'Form',
    profile: 'Profil',
    preferences: 'Vorlieben',
};
const TEAM_DETAIL_PAGE_ORDER = ['skills', 'form', 'profile', 'preferences'];
const RIDER_MENU_PAGE_SIZE = 50;
function getActiveTeamTableColumns() {
    return [...TEAM_TABLE_COLUMNS, ...TEAM_DETAIL_PAGE_COLUMNS[state.teamDetailPage]];
}
function getActiveRiderMenuTableColumns() {
...
function renderSkillValue(value, potential) {
...
function renderSkillValueWithDelta(value, yearStart, potential) {
    if (yearStart == null) {
...
function renderRaceFormBonusValue(value) {
...
function renderSeasonFormValue(value) {
    const amount = value ?? 0;
    if (amount === 0) {
        return '0,0';
    }
...
function renderLoadMalusValue(value, warning = 'none', title) {
    const amount = value ?? 0;
...
function renderRiderProgramButton(rider) {
...
function formatElevationGain(value) {
...
function stageProfileOptionsHtml(selected) {
    return STAGE_PROFILE_OPTIONS.map((profile) => `<option value="${profile}"${profile === selected ? ' selected' : ''}>${esc(profile)}</option>`).join('');
}
function terrainOptionsHtml(selected) {
    return STAGE_TERRAIN_OPTIONS.map((terrain) => `<option value="${terrain}"${terrain === selected ? ' selected' : ''}>${esc(terrain)}</option>`).join('');
}
function markerTypeOptionsHtml(selected, scope = 'start', segmentIndex = 0, totalSegments = 1) {
    ...
    return optionTypes.map((markerType) => `<option value="${markerType}"${markerType === selected ? ' selected' : ''}>${esc(markerType)}</option>`).join('');
}
function markerCategoryOptionsHtml(selected) {
    return ['<option value="">–</option>', ...STAGE_MARKER_CATEGORIES.map((category) => `<option value="${category}"${category === selected ? ' selected' : ''}>${esc(category)}</option>`),
    ].join('');
}
function markerTypeSortValue(markerType) {
    return STAGE_MARKER_TYPES.indexOf(markerType);
}
function normalizeIncomingMarkerType(markerType) {
    if (!markerType)
        return null;
    if (markerType === 'finish')
        return 'finish_flat';
    return STAGE_MARKER_TYPES.includes(markerType) ? markerType : null;
}
function normalizeWaypointMarkers(waypoint) {
...
function normalizeSegmentMarkers(markers) {
    return sortStageMarkers(Array.isArray(markers) ? markers : []);
}
function buildStageEditorSegmentsFromWaypoints(waypoints) {
...
function buildStageEditorWaypointsFromSegments(segments) {
...
function recalculateStageEditorSegmentStartElevations(draft) {
...
function normalizeStageEditorDraft(draft) {
    const normalizedDraft = {
        ...draft,
        segments: (draft.segments?.length
            ? draft.segments
            : buildStageEditorSegmentsFromWaypoints(draft.waypoints ?? [])).map((segment) => ({
            ...segment,
...
function markerInputValue(markers, key) {
    return markers.map((marker) => marker[key] ?? 'null').join('|');
}
function markerLabelValue(markers) {
    return markers.map((marker) => marker.type).join(' | ');
}
function describeMarkerScope(scope, segmentIndex, totalSegments) {
    if (scope === 'start') {
...
function collectStageEditorClimbPairIssues(draft) {
    const issuesBySegment = new Map();
    const openClimbs = [];
    const pushIssue = (segmentIndex, issue) => {
...
function normalizeMarkerForType(marker) {
...
function getStageEditorSegmentIssuesAt(draft, segmentIndex) {
...
function stageEditorFieldErrorClass(hasError) {
    return hasError ? ' stage-editor-input-invalid' : '';
}
function renderStageEditorSegmentIssues(draft, segmentIndex) {
...
function renderSegmentMarkerBlock(markers, segmentIndex, totalSegments, scope) {
...
function roundStageEditorOneDecimal(value) {
    return Math.round(value * 10) / 10;
}
function roundStageEditorKm(value) {
    return Number(value.toFixed(2));
}
function syncStageEditorDraftStats(draft) {
...
function classifyStageEditorClimb(distanceKm, gainMeters, avgGradient) {
...
function detectStageEditorClimbs(waypoints) {
    const climbs = [];
    let startIndex = null;
    let topIndex = null;
    let descentMeters = 0;
...
function suggestStageEditorProfile(draft) {
    ...
    if (hasCobble)
        return 'Cobble';
    if (draft.totalDistanceKm <= 25 && draft.elevationGainMeters < 250)
        return 'ITT';
    if (hasHcOrCat1 && draft.elevationGainMeters >= 2800)
        return 'High_Mountain';
    if (hasHcOrCat1 || draft.elevationGainMeters >= 1800)
        return 'Mountain';
    if (draft.elevationGainMeters >= 1100)
        return 'Medium_Mountain';
    if (draft.elevationGainMeters >= 700)
        return 'Hilly';
    if (draft.elevationGainMeters >= 350)
        return 'Rolling';
    return 'Flat';
}
function isManualStageEditorTerrain(terrain) {
    return terrain === 'Cobble' || terrain === 'Cobble_Hill' || terrain === 'Sprint';
}
function classifyAutoClimbTerrain(climb) {
...
function applyAutomaticStageEditorTerrain(draft) {
    ...
    let descentStartIndex = null;
    let descentDistanceKm = 0;
    const commitDescent = (endExclusive) => {
...
function syncStageEditorDerivedState(draft) {
...
function ensureStageEditorBoundaryMarkers(draft) {
...
function renderSegmentMarkerRows(markers, segmentIndex, totalSegments, scope) {
    if (markers.length === 0) {
    ...
    return `<div class="stage-editor-marker-list">${markers.map((marker, markerIndex) => {
      ...
      <div class="stage-editor-marker-row">
        ...
        <button type="button" class="btn btn-danger btn-xs" data-segment-action="remove-marker" data-marker-scope="${scope}" data-marker-index="${markerIndex}" data-segment-index="${segmentIndex}" ${canRemove ? '' : 'disabled'}>âœ•</button>
      </div>`;
    }).join('')}</div>`;
}
function updateStageEditorMarker(segmentIndex, markerIndex, scope, field, rawValue) {
    ...
    if (field === 'markerType') {
        ...
        marker.cat = rawValue ? rawValue : null;
    }
    if (scope === 'start') {
...
function addStageEditorMarker(segmentIndex, scope) {
    if (!state.stageEditorDraft)
        return;
    const segment = state.stageEditorDraft.segments[segmentIndex];
    if (!segment)
    ...
    const nextMarker = scope === 'start'
        ...
        : (segmentIndex === state.stageEditorDraft.segments.length - 1 && !segment.endMarkers.some((marker) => isFinishMarkerType(marker.type))
            ...
            : { type: 'sprint_intermediate', name: null, cat: 'Sprint' });
    if (scope === 'start') {
...
function removeStageEditorMarker(segmentIndex, markerIndex, scope) {
    if (!state.stageEditorDraft)
        return;
    const segment = state.stageEditorDraft.segments[segmentIndex];
    if (!segment)
...
function initializeStageEditorForm() {
    $('stage-editor-profile').innerHTML = stageProfileOptionsHtml('Flat');
...
function getStageEditorSegmentStartKm(draft, index) {
    let kmMark = 0;
...
function getStageEditorSegmentEndElevation(segment) {
    return Math.round(segment.startElevation + ((segment.lengthKm * 1000) * (segment.gradientPercent / 100)));
}
function resolveFirstFreePositiveInteger(values) {
...
function resolveNextFreeStageEditorStageId() {
    return resolveFirstFreePositiveInteger(state.stageEditorExistingStages.map((stage) => stage.stageId));
}
function resolveNextFreeStageEditorRaceId() {
    return resolveFirstFreePositiveInteger([
        ...state.stageEditorExistingStages.map((stage) => stage.raceId),
        ...state.races.map((race) => race.id),
    ]);
}
function setStageEditorDefaults(draft) {
    const profileSelect = $('stage-editor-profile');
    profileSelect.innerHTML = stageProfileOptionsHtml(draft.suggestedProfile);
    profileSelect.value = draft.suggestedProfile;
    $('stage-editor-stage-id').value = String(resolveNextFreeStageEditorStageId());
    $('stage-editor-race-id').value = String(resolveNextFreeStageEditorRaceId());
    const detailsFileInput = $('stage-editor-details-file');
    if (!detailsFileInput.value.trim()) {
        detailsFileInput.value = `${slugifyFileName(draft.routeName)}.csv`;
    }
    const dateInput = $('stage-editor-date');
    if (!dateInput.value && state.gameState?.currentDate) {
        dateInput.value = state.gameState.currentDate;
    }
}
function setStageEditorMetadataFields(metadata) {
    $('stage-editor-stage-id').value = String(metadata.stageId);
    $('stage-editor-race-id').value = String(metadata.raceId);
    $('stage-editor-stage-number').value = String(metadata.stageNumber);
    $('stage-editor-date').value = metadata.date;
    $('stage-editor-details-file').value = metadata.detailsCsvFile;
    const profileSelect = $('stage-editor-profile');
    profileSelect.innerHTML = stageProfileOptionsHtml(metadata.profile);
    profileSelect.value = metadata.profile;
    $('stage-editor-final-spread-start').value = String(metadata.finalSpreadStartPercent);
    $('stage-editor-final-push-start').value = String(metadata.finalPushStartPercent);
    $('stage-editor-final-spread-difficulty').value = String(metadata.finalSpreadDifficultyMultiplier);
    $('stage-editor-crash-multiplier').value = String(metadata.crashIncidentMultiplier);
    $('stage-editor-mechanical-multiplier').value = String(metadata.mechanicalIncidentMultiplier);
}
function renderStageEditorExistingStages() {
    const select = $('stage-editor-existing-stage');
    const button = $('btn-stage-editor-load-existing');
    ...
    if (!state.stageEditorExistingStagesLoaded) {
        ...
        hint.textContent = 'CSV-Stages werden beim Ööffnen des Editors geladen.';
        return;
    }
    if (state.stageEditorExistingStages.length === 0) {
        select.innerHTML = '<option value="">Keine CSV-Stages gefunden</option>';
        button.disabled = true;
        hint.textContent = 'Keine Einträge in data/csv/stages.csv gefunden.';
        return;
    }
    ...
    hint.textContent = `${state.stageEditorExistingStages.length} CSV-Stages verfügbar. Export lädt neue Dateien herunter und schreibt nichts automatisch.`;
}
async function loadStageEditorExistingStages(force = false) {
...
function renderStageEditorScoreBadge(score, minScore, maxScore) {
    let relativeRatio = 0;
...
function resolveStageEditorCategoryClassName(category) {
...
function renderStageEditorCategoryBadge(category) {
...
function renderStageEditorProfileOpenButton(content, stageId, title, climbId) {
...
function renderStageEditorStageScorePopover(row) {
...
function renderStageEditorClimbScorePopover(row) {
...
function renderStageEditorScoreControl(score, minScore, maxScore, stageId, popoverContent, title, climbId) {
    return `
...
function renderStageEditorStagesOverview() {
...
function renderStageEditorClimbsOverview() {
...
async function loadStageEditorOverview(force = false) {
...
function updateStageEditorSegment(index, field, rawValue) {
    if (!state.stageEditorDraft)
        return;
    ...
    if (field === 'segmentLengthKm') {
        ...
        if (!Number.isFinite(nextLengthKm))
            return;
    ...
    if (field === 'segmentGradientPercent') {
        ...
        if (!Number.isFinite(nextGradientPercent) || segment.lengthKm <= 0)
            return;
...
function getStageEditorIssues(draft) {
    if (!draft)
        return ['Noch keine Strecke importiert.'];
    const issues = [];
    if (draft.segments.length === 0) {
...
function getStageEditorMetadataErrors() {
    const errors = [];
    const stageId = Number($('stage-editor-stage-id').value);
    const raceId = Number($('stage-editor-race-id').value);
    const stageNumber = Number($('stage-editor-stage-number').value);
    const date = $('stage-editor-date').value.trim();
    const detailsFile = $('stage-editor-details-file').value.trim();
    const finalSpreadStartPercent = Number($('stage-editor-final-spread-start').value);
    const finalPushStartPercent = Number($('stage-editor-final-push-start').value);
    const finalSpreadDifficultyMultiplier = Number($('stage-editor-final-spread-difficulty').value);
    const crashIncidentMultiplier = Number($('stage-editor-crash-multiplier').value);
    const mechanicalIncidentMultiplier = Number($('stage-editor-mechanical-multiplier').value);
    if (!Number.isInteger(stageId) || stageId <= 0)
        errors.push('Stage-ID fehlt oder ist ungültig.');
    if (!Number.isInteger(raceId) || raceId <= 0)
        errors.push('Race-ID fehlt oder ist ungültig.');
    if (!Number.isInteger(stageNumber) || stageNumber <= 0)
        errors.push('Etappennummer fehlt oder ist ungültig.');
...
function readStageEditorMetadata() {
    return {
        stageId: Number($('stage-editor-stage-id').value),
        raceId: Number($('stage-editor-race-id').value),
        stageNumber: Number($('stage-editor-stage-number').value),
        date: $('stage-editor-date').value.trim(),
        profile: $('stage-editor-profile').value,
        detailsCsvFile: $('stage-editor-details-file').value.trim(),
        startElevation: state.stageEditorDraft?.segments[0]?.startElevation ?? 0,
        finalSpreadStartPercent: Number($('stage-editor-final-spread-start').value),
        finalPushStartPercent: Number($('stage-editor-final-push-start').value),
        finalSpreadDifficultyMultiplier: Number($('stage-editor-final-spread-difficulty').value),
        crashIncidentMultiplier: Number($('stage-editor-crash-multiplier').value),
        mechanicalIncidentMultiplier: Number($('stage-editor-mechanical-multiplier').value),
    };
}
...
function renderStageEditorChart(draft) {
...
function renderStageEditor() {
    ...
    const exportButton = $('btn-stage-editor-export');
    if (!draft) {
        summary.innerHTML = '';
        warnings.innerHTML = '';
    ...
    <tr data-segment-index="${index}" class="${getStageEditorSegmentIssuesAt(draft, index).length > 0 ? 'stage-editor-segment-row-invalid' : ''}">
      ...
      <td class="stage-editor-row-actions">
        ...
        ${draft.segments.length > 1 ? `<button type="button" class="btn btn-danger btn-xs" data-segment-action="delete" data-segment-index="${index}">âœ•</button>` : ''}
      </td>
    </tr>`).join('');
    ...
    exportHint.textContent = alertItems.length > 0
        ...
        : `Exportiert ${$('stage-editor-details-file').value || 'stage_details.csv'} und eine stages-Row als Download.`;
}
function updateStageEditorWaypoint(index, field, rawValue) {
    if (!state.stageEditorDraft)
        return;
    const segment = state.stageEditorDraft.segments[index];
    if (!segment)
...
function insertStageEditorWaypoint(index) {
    if (!state.stageEditorDraft)
        return;
    ...
    if (firstLengthKm < STAGE_EDITOR_MIN_SEGMENT_KM || secondLengthKm < STAGE_EDITOR_MIN_SEGMENT_KM)
        return;
...
function appendStageEditorWaypoint() {
    if (!state.stageEditorDraft)
        return;
...
function deleteStageEditorWaypoint(index) {
    if (!state.stageEditorDraft)
        return;
    if (state.stageEditorDraft.segments.length <= 1)
        return;
...
async function onStageEditorImport() {
    const input = $('stage-editor-file');
    ...
    showLoading('Route wird importiertâ€¦â€¦');
    try {
...
async function onStageEditorLoadExisting() {
    const stageId = Number($('stage-editor-existing-stage').value);
    if (!Number.isInteger(stageId) || stageId <= 0) {
...
async function onStageEditorExport() {
    if (!state.stageEditorDraft) {
    ...
    showLoading('CSV-Dateien werden erstelltâ€¦â€¦');
    try {
...
function renderRiderAvailabilityMarker(rider) {
    ...
    return `<span class="rider-availability-marker" title="${esc(title)}" aria-label="${esc(title)}">âœ‚</span>`;
}
function getRiderRoleName(rider) {
    return rider.role?.name ?? (rider.roleId != null ? `Rolle ${rider.roleId}` : '–');
}
function getTeamTopAverage(teamId, limit = 12) {
...
function getPeakDate(rider, index) {
    return rider.seasonFormPeakDates?.[index];
}
function getSpecializationSortLabel(value) {
    return value == null ? undefined : getRiderSpecializationLabel(value);
}
function getDefaultTeamSortDirection(sortKey) {
...
function getRiderMenuSortIndicator(sortKey) {
...
function getRaceParticipantsSortIndicator(sortKey) {
...
function renderRaceParticipantsHeader(label, sortKey, title) {
...
function renderStageEditorOverviewHeader(label, sortKey, activeKey, direction, table) {
...
function compareStageEditorClimbRows(left, right) {
...
function sortStageEditorStageRows(rows) {
...
function sortStageEditorClimbRows(rows) {
...
function getRaceParticipantPhaseRank(participant) {
    const order = { rise: 0, neutral: 1, fall: 2 };
    return order[participant.rider.seasonFormPhase ?? 'neutral'] ?? order['neutral'];
}
function sortRaceParticipants(participants) {
...
function renderTeamTableHeader(column) {
    if (!column.sortKey) {
...
function renderRiderMenuTableHeader(column) {
    if (!column.sortKey) {
        return `<th class="${column.className ?? ''}"></th>`;
    }
...
function renderTeamDetailPageTabs() {
    return `
...
function renderRiderMenuDetailPageTabs() {
    return `
...
function renderTeamCountryCell(rider) {
...
function renderRiderTags(rider) {
    const tags = [
    ...
    return tags.length > 0 ? tags.map(esc).join(', ') : '–';
}
function renderRiderRaceFocus(rider) {
    const focus = [
...
function renderTeamTableCell(rider, column) {
    switch (column.id) {
        ...
        case 'peak1':
            return `<td>${esc(getPeakDate(rider, 0) ?? '–')}</td>`;
        case 'peak2':
            return `<td>${esc(getPeakDate(rider, 1) ?? '–')}</td>`;
        case 'peak3':
            return `<td>${esc(getPeakDate(rider, 2) ?? '–')}</td>`;
        case 'riderType':
            return `<td>${esc(getRiderSpecializationLabel(rider.riderType))}</td>`;
        case 'specialization1':
            return `<td>${esc(rider.specialization1 ? getRiderSpecializationLabel(rider.specialization1) : '–')}</td>`;
        case 'specialization2':
            return `<td>${esc(rider.specialization2 ? getRiderSpecializationLabel(rider.specialization2) : '–')}</td>`;
        case 'specialization3':
            return `<td>${esc(rider.specialization3 ? getRiderSpecializationLabel(rider.specialization3) : '–')}</td>`;
        case 'tags':
        ...
        case 'skillDevelopment':
            return `<td>${rider.skillDevelopment ?? '–'}</td>`;
        case 'favoriteRaces':
...
function getTeamSortLabel(sortKey) {
    ...
    if (sortKey === 'longTermFatigueMalus')
        return 'Langzeit-Erschöpfung';
    if (sortKey === 'shortTermFatigueMalus')
        return 'Akuter Verschleiß';
...
function sortTeamRiders(riders) {
...
function sortRiderMenuRiders(riders) {
    const sortedRiders = [...riders];
...
function renderRacePrefs(raceIds) {
    if (raceIds.length === 0)
        return '–';
...
function renderPeakDatesSummary(rider) {
...
const RIDER_TEAM_EDITOR_COLUMNS = [
...
function resolveRiderTeamEditorTeamKey(teamId) {
    return teamId == null ? 'free-agents' : String(teamId);
}
function getRiderTeamEditorTeamName(teamId) {
...
function calculateEditorOverall(rider) {
    const values = [
        rider.skillFlat,
        rider.skillMountain,
...
function getDefaultRiderTeamEditorSortDirection(sortKey) {
    return [
        'riderId',
        'countryId',
        'birthYear',
...
function getRiderTeamEditorSortIndicator(sortKey) {
...
function renderRiderTeamEditorHeader(column) {
...
function compareRiderTeamEditorRows(left, right) {
...
function sortRiderTeamEditorRows(rows) {
    const direction = state.riderTeamEditorSort.direction === 'asc' ? 1 : -1;
    return [...rows].sort((left, right) => ((compareRiderTeamEditorRows(left, right)
...
function getFilteredRiderTeamEditorRows(payload) {
...
function renderRiderTeamEditorTeamOptions(teamId) {
    const payload = state.riderTeamEditorPayload;
    if (!payload) {
...
function isDirtyRiderTeamEditorRow(riderId) {
    return state.riderTeamEditorDirtyRiderIds.includes(riderId);
}
function renderRiderTeamEditorCell(rider, column) {
    ...
    switch (column.inputType) {
        ...
        case 'text': {
            const value = String(rider[column.key] ?? '');
...
function renderRiderTeamEditorSidebar(payload) {
...
function renderRiderTeamEditor() {
        ...
        <div class="team-detail-card">
          <div class="rider-team-editor-toolbar">
            ...
            <div class="rider-team-editor-actions">
              ...
              <button type="button" class="btn btn-secondary" data-rider-team-editor-action="export" ${state.riderTeamEditorExporting ? 'disabled' : ''}>${state.riderTeamEditorExporting ? 'Exportiertâ€¦' : 'riders.csv exportieren'}</button>
              <button type="button" class="btn btn-primary" data-rider-team-editor-action="save" ${dirtyCount === 0 || state.riderTeamEditorSaving ? 'disabled' : ''}>${state.riderTeamEditorSaving ? 'Speichertâ€¦' : 'Änderungen speichern'}</button>
            </div>
          </div>
...
function formatFormDebugValue(value) {
    const amount = value ?? 0;
    const prefix = amount > 0 ? '+' : '';
    return `${prefix}${amount.toFixed(2).replace('.', ',')}`;
}
function buildFormSparklinePath(points, width, height, minValue, maxValue) {
    if (points.length === 0) {
        return '';
    }
...
function renderRiderFormSparkline(rider) {
...
function getRiderSpecializationLabel(value) {
    switch (value) {
        case 'Berg':
            return 'Bergfahrer';
        case 'Hill':
...
function renderRiderStatsCategoryBadge(categoryName) {
...
function resolveRiderStatsFinalTypeClassName(rowType) {
    switch (rowType) {
        case 'gc_final':
            return 'is-gc';
        case 'points_final':
...
function getRiderStatsRowTypeLabel(rowType) {
    switch (rowType) {
        case 'gc_final':
...
function renderRiderStatsFinalTypeBadge(rowType) {
...
function formatRiderStatsRaceBlockMeta(block) {
...
function resolveCurrentSeasonRank(riderId) {
    if (riderId == null || state.riders.length === 0) {
        return null;
    }
    const orderedRiders = [...state.riders].sort((left, right) => ((right.seasonPoints ?? 0) - (left.seasonPoints ?? 0)
...
function getRiderStatsFinalTypeSortOrder(rowType) {
    switch (rowType) {
        case 'gc_final':
            return 0;
        case 'points_final':
...
function sortRiderStatsRowsNewestFirst(rows) {
...
function sortRiderStatsRaceBlocksNewestFirst(blocks) {
    return [...blocks]
        .map((block) => ({
        ...block,
...
function formatRiderStatsCareerRaceDaysTooltip(payload) {
...
function formatRiderStatsAvailabilityTitle(isUnavailable, healthStatusLabel, unavailableUntil, unavailableDaysRemaining) {
    if (!isUnavailable) {
        return null;
    }
...
function renderRiderStatsAvailabilityMarker(rider, payload) {
    const title = formatRiderStatsAvailabilityTitle(payload?.isUnavailable ?? rider?.isUnavailable === true, payload?.healthStatusLabel ?? rider?.healthStatusLabel ?? null, payload?.unavailableUntil ?? rider?.unavailableUntil ?? null, payload?.unavailableDaysRemaining ?? rider?.unavailableDaysRemaining ?? 0);
    if (!title) {
        return '';
    }
    return `<span class="rider-availability-marker" title="${esc(title)}" aria-label="${esc(title)}">ÃƒÆ’Ã‚Â¢Ãƒâ€¦Ã¢â‚¬Å“Ãƒâ€¦Ã‚Â¡</span>`;
}
function renderRiderStatsTitle(rider, payload) {
...
function renderRiderStatsHeatPill(label, value, maxValue) {
...
function renderRiderStatsTerrainSources(payload) {
...
function renderRiderStatsRaceFormatSources(payload) {
...
function renderRiderStatsTabs(payload) {
...
function renderRiderStatsProgramTab(payload) {
...
function renderRiderStatsRankBadge(label, variant) {
    return `<span class="rider-stats-rank-badge rider-stats-rank-badge-${variant}">${esc(label)}</span>`;
}
function renderRiderStatsBreakaway(row) {
    if (!row.isBreakaway || row.rowType !== 'stage_result' || row.finishStatus !== 'classified') {
        return '–';
    }
    return '<span class="rider-stats-breakaway-icon" aria-label="Ausreißer" title="Ausreißer">ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“Ãƒâ€šÃ‚Â¼</span>';
}
function renderRiderStatsPlacement(row) {
...
function renderRiderStatsBody(rider, payload, isLoading = false) {
    const teamName = rider?.activeTeamId != null
        ? state.teams.find((team) => team.id === rider.activeTeamId)?.name ?? null
        : null;
...
async function openRiderStats(riderId) {
    const rider = findRiderById(riderId);
    const teamName = rider?.activeTeamId != null
        ? state.teams.find((team) => team.id === rider.activeTeamId)?.name ?? null
        : null;
...
function showLoading(msg = 'Ladeâ€¦') {
    $('loading-msg').textContent = msg;
    $('loading-progress').classList.add('hidden');
    $('loading-overlay').classList.remove('hidden');
}
function hideLoading() { $('loading-overlay').classList.add('hidden'); }
function showInstantProgress(progress) {
    $('loading-progress').classList.remove('hidden');
    $('loading-overlay').classList.remove('hidden');
    updateInstantProgress(progress);
}
function updateInstantProgress(progress) {
    const percent = Math.round(Math.min(1, Math.max(0, progress)) * 100);
    $('loading-msg').textContent = `Instant-Simulation läuft â€¦ ${percent}%`;
...
function hideError(elemId) { $(elemId).classList.add('hidden'); }
...
function formatPendingStageLabel(raceName, stageNumber, profile) {
    return `${raceName} ·· Etappe ${stageNumber} · ${profile}`;
}
function buildRealtimeCommitEntries(snapshot, bootstrap) {
...
async function openInstantStage(stageId) {
    ...
    try {
        ...
        alert('Unerwarteter Fehler bei der Instant-Simulation: ' + error.message);
    }
    finally {
...
function canEditPendingStage(stage) {
    return stage.stageId > 0;
}
function getRosterEditorSelectedCount(teamId) {
...
function isRosterEditorSelectionValid() {
    if (!state.rosterEditor)
        return false;
...
function renderRosterEditor() {
    ...
    const applyButton = $('btn-apply-roster-editor');
...
function startRealtimeSimulation(bootstrap, activateLiveView) {
...
async function openRosterEditor(stageId) {
    ...
    try {
        ...
        state.rosterEditor = null;
        state.rosterEditorSelectedRiderIds = [];
        showError('roster-editor-error', error.message);
        showModal('rosterEditor');
        renderRosterEditor();
    }
    finally {
...
async function applyRosterEditor() {
    const payload = state.rosterEditor;
    if (!payload) {
        return;
    }
...
function renderRealtimeRaceView() {
    const select = $('race-sim-stage-select');
...
async function openRealtimeStage(stageId, activateLiveView) {
    ...
    try {
        ...
        renderRealtimeRaceView();
        alert('Unerwarteter Fehler bei der Live-Simulation: ' + error.message);
    }
    finally {
        if (realtimeStageLoadInFlightId === stageId) {
...
function formatResultsStageLabel(race, stage) {
...
async function onLoadSave(filename) {
    showLoading('Karriere wird geladenâ€¦');
    const res = await api.loadSave(filename);
    hideLoading();
    if (!res.success) {
        alert('Fehler beim Laden: ' + res.error);
        return;
    }
    state.currentSave = res.data ?? null;
    await enterGameScreen();
}
async function onDeleteSave(filename, name) {
    if (!confirm(`Karriere "${name}" wirklich löschen?`))
        return;
    showLoading('Löschenâ€¦');
...
async function onDeleteAllSaves() {
    const res = await api.listSaves();
    ...
    if (!confirm(`Wirklich alle ${saves.length} Karrieren löschen?`))
        return;
    if (!confirm('Dieser Schritt löscht alle Spielstände dauerhaft. Wirklich fortfahren?'))
        return;
    showLoading('Alle Karrieren werden gelöschtâ€¦');
    try {
...
$('btn-new-career').addEventListener('click', async () => {
    ...
    const select = $('input-team-id');
    select.innerHTML = '<option value="">Wird geladenâ€¦</option>';
    showModal('newCareer');
    ...
    select.innerHTML = res.data.map(t => `<option value="${t.id}">${esc(t.name)} (${esc(t.division ?? t.divisionName ?? '')})</option>`).join('');
});
...
$('roster-editor-body').addEventListener('click', (event) => {
    const button = event.target.closest('button[data-roster-rider-id]');
    if (!button || button.disabled || !state.rosterEditor)
        return;
    const teamId = Number(button.dataset['rosterTeamId']);
    const riderId = Number(button.dataset['rosterRiderId']);
    if (!Number.isFinite(teamId) || !Number.isFinite(riderId))
        return;
    const team = state.rosterEditor.teams.find((entry) => entry.team.id === teamId);
    if (!team)
        return;
...
$('btn-confirm-new').addEventListener('click', async () => {
    const careerName = ($('input-career-name')).value.trim();
    const teamIdVal = ($('input-team-id')).value;
    ...
    hideError('new-career-error');
    showLoading('Neue Karriere wird erstelltâ€¦');
    const res = await api.createSave(filename, careerName, teamId);
    if (!res.success) {
        hideLoading();
        showError('new-career-error', res.error ?? 'Unbekannter Fehler.');
        return;
    }
...
$('saves-list').addEventListener('click', async (event) => {
    ...
    if (!filename)
        return;
    if (saveAction === 'load') {
        await onLoadSave(filename);
        return;
    }
    if (saveAction === 'delete') {
        await onDeleteSave(filename, careerName ?? filename);
    }
});
// ============================================================
//  Game Screen
...
async function enterGameScreen() {
    ...
    showLoading('Spiel wird geladenâ€¦');
    try {
        await loadGameState();
        await loadRaces();
        renderDashboard();
...
document.querySelectorAll('.nav-btn').forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        if (view === 'dashboard')
            renderDashboard();
        if (view === 'teams')
            void refüreshTeamsViewData(); // immer neu laden bei Nav-Klick
        if (view === 'riders')
            void refüreshTeamsViewData();
...
$('stage-editor-stages-table').addEventListener('click', (event) => {
    const sortButton = event.target.closest('button[data-stage-editor-stages-sort]');
    if (!sortButton)
        return;
    const sortKey = sortButton.dataset['stageEditorStagesSort'];
...
$('stage-editor-stages-table').addEventListener('click', async (event) => {
    const profileButton = event.target.closest('button[data-stage-profile-open-stage-id]');
    if (!profileButton) {
        return;
    }
...
$('stage-editor-climbs-table').addEventListener('click', (event) => {
    const sortButton = event.target.closest('button[data-stage-editor-climbs-sort]');
    if (!sortButton)
        return;
    const sortKey = sortButton.dataset['stageEditorClimbsSort'];
...
$('view-rider-team-editor').addEventListener('click', (event) => {
    const sortButton = event.target.closest('button[data-rider-team-editor-sort]');
    if (sortButton) {
        const sortKey = sortButton.dataset['riderTeamEditorSort'];
        if (state.riderTeamEditorSort.key === sortKey) {
    ...
    const filterButton = event.target.closest('button[data-rider-team-editor-team-filter]');
    if (filterButton) {
        state.riderTeamEditorSelectedTeamKey = filterButton.dataset['riderTeamEditorTeamFilter'] ?? '';
        renderRiderTeamEditor();
        return;
    }
    const actionButton = event.target.closest('button[data-rider-team-editor-action]');
    if (actionButton) {
...
$('view-rider-team-editor').addEventListener('change', (event) => {
    const filterSelect = event.target.closest('#rider-team-editor-team-select');
    if (filterSelect) {
        state.riderTeamEditorSelectedTeamKey = filterSelect.value;
        renderRiderTeamEditor();
        return;
    }
    const fieldInput = event.target.closest('[data-rider-team-editor-field][data-rider-team-editor-rider-id]');
    if (!fieldInput) {
        return;
    }
    const riderId = Number(fieldInput.dataset['riderTeamEditorRiderId']);
    const field = fieldInput.dataset['riderTeamEditorField'];
...
$('rider-stats-body').addEventListener('click', (event) => {
    const tabButton = event.target.closest('button[data-rider-stats-tab]');
    if (!tabButton) {
        return;
    }
    const nextTab = tabButton.dataset['riderStatsTab'];
...
$('pending-stages-list').addEventListener('click', (event) => {
    const editButton = event.target.closest('button[data-edit-stage-roster]');
    if (editButton) {
    ...
    const liveButton = event.target.closest('button[data-live-stage]');
    if (liveButton) {
    ...
    const instantButton = event.target.closest('button[data-instant-stage]');
    if (instantButton) {
...
$('race-sim-stage-select').addEventListener('change', (event) => {
...
$('results-race-select').addEventListener('change', (event) => {
...
$('dashboard-races-tbody').addEventListener('click', (event) => {
    const participantsButton = event.target.closest('button[data-dashboard-race-participants-id]');
    ...
    const raceButton = event.target.closest('button[data-dashboard-race-id]');
    if (!raceButton) {
        return;
    }
...
$('race-stages-body').addEventListener('click', (event) => {
    const profileButton = event.target.closest('button[data-dashboard-stage-profile-id]');
    if (!profileButton) {
        return;
    }
...
$('race-participants-body').addEventListener('click', (event) => {
    ...
    const sortButton = event.target.closest('button[data-race-participants-sort]');
    if (!sortButton) {
        return;
    }
    const sortKey = sortButton.dataset['raceParticipantsSort'];
    if (state.raceParticipantsSort.key === sortKey) {
...
$('results-stage-select').addEventListener('change', (event) => {
    const stageId = Number(event.target.value);
...
$('results-type-tabs').addEventListener('click', (event) => {
    const button = event.target.closest('button[data-result-type-id]');
    if (!button)
        return;
    const resultTypeId = Number(button.dataset['resultTypeId']);
    if (!Number.isFinite(resultTypeId))
        return;
...
$('results-type-tabs').addEventListener('click', (event) => {
    const button = event.target.closest('button[data-results-special-view]');
    if (!button)
        return;
    const specialView = button.dataset['resultsSpecialView'];
    if (specialView !== RESULTS_NON_FINISHERS_KEY)
        return;
...
$('results-marker-tabs').addEventListener('click', (event) => {
    const button = event.target.closest('button[data-marker-key]');
    if (!button)
        return;
...
$('stage-editor-waypoints').addEventListener('change', (event) => {
    ...
    const row = target.closest('tr[data-segment-index]');
    ...
    if (field === 'markerType' || field === 'markerName' || field === 'markerCat') {
        ...
        if (!Number.isInteger(markerIndex))
            return;
        if (markerScope !== 'start' && markerScope !== 'end')
            return;
...
$('btn-advance-day').addEventListener('click', async () => {
    ...
    try {
        ...
        if (state.currentSave && res.data)
            state.currentSave.currentSeason = res.data.season;
        await loadGameState();
        await loadRaces();
        ...
        alert('Unerwarteter Fehler beim Tageswechsel: ' + e.message);
    }
    finally {
        hideLoading();
    }
...
async function loadGameState() {
    ...
    if (!gameStateRes.success) {
        console.error(gameStateRes.error);
        return;
    }
    ...
    if (state.currentSave && gameStateRes.data)
        state.currentSave.currentSeason = gameStateRes.data.season;
}
function renderGameState() {
    ...
    const advanceButton = $('btn-advance-day');
...
async function completeRealtimeStage(stageId, entries, markerClassifications, incidents) {
    ...
    try {
        ...
        alert('Unerwarteter Fehler beim Speichern des Live-Ergebnisses: ' + error.message);
    }
    finally {
...
async function loadStageResults(stageId, silentIfMissing) {
...
async function loadDraftHistory(season, silent = false) {
...
let currentDraftSort = { key: 'pickNumber', asc: true };
  ...
  `;
    for (const row of sortedRows) {
      ...
      <tr>
        ...
        <td class="text-center"><span class="rider-stats-summary-pill rider-stats-summary-pill-heat ${getHeatmapClassForRating(row.overallAtDraft)}">${row.overallAtDraft.toFixed(1)}</span></td>
        <td class="text-center"><span class="rider-stats-summary-pill rider-stats-summary-pill-heat ${getHeatmapClassForRating(row.potOverallAtDraft)}">${row.potOverallAtDraft.toFixed(1)}</span></td>
      </tr>
    `;
    }
    html += `</tbody></table>`;
...
let currentInjuriesSort = { key: 'unavailableDays', asc: false };
function renderInjuriesView() {
    ...
    window.setInjuriesSort = setSort;
    let html = `
    <table class="data-table">
      <thead>
        <tr>
  ...
  `;
    ...
    for (const row of sortedRows) {
        ...
        if (row.teamAbbreviation) {
            teamHtml = `<div style="display:flex; align-items:center; justify-content:center; gap:0.5rem;">${renderMiniJersey(row.teamId, row.teamAbbreviation)} ${esc(row.teamAbbreviation)}</div>`;
        }
...
function resolveParticipantCountryCode(row) {
    if (row.riderId != null) {
        const rider = findRiderById(row.riderId);
        if (rider?.country?.code3)
            return rider.country.code3;
    ...
    if (row.teamId != null) {
        ...
        if (team?.countryCode)
            return team.countryCode;
        if (team?.country?.code3)
            return team.country.code3;
    }
    return null;
}
function renderResultsView() {
    const raceSelect = $('results-race-select');
    ...
    const selectedClassification = state.stageResults?.classifications.find((classification) => classification.resultTypeId === state.selectedResultTypeId) ?? state.stageResults?.classifications[0] ?? null;
    ...
    const previousGcRanks = new Map((state.stageResults.previousGcStandings ?? []).map((standing) => [standing.riderId, standing.rank]));
  ...
  `;
    ...
    tbody.innerHTML = showNonFinishers
      ...
      <tr>
        ...
        <td class="results-flag-col-cell">${renderResultsFlagColumn(row.countryCode)}</td>
        <td>${esc(row.teamName || 'ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â')}</td>
    ...
    `).join('') || '<tr><td colspan="7" class="results-empty-cell">Keine OTL/DNF bis zu dieser· Etappe.</td></tr>'
        : showStageOverviewTable && selectedClassification
            ? selectedClassification.rows.map((row) => {
                ...
                const teamName = row.riderName ? row.teamName : 'ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â';
...
async function loadSeasonStandings(silent) {
...
function renderSeasonStandingsView() {
      ...
      <tr>
        ...
        : standardRows.map((row) => {
            ...
            const secondary = state.selectedSeasonStandingScope === 'teams'
                ? (row.countryName ?? row.countryCode ?? 'ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â')
                : row.teamName;
            return `
        <tr>
          <td class="pos-${Math.min(row.rank, 3)}">${row.rank}</td>
...
function renderDashboardRaces() {
    ...
    tbody.innerHTML = visibleRaces.map(race => {
        ...
        const statusBadge = isDone
            ...
            : isLive
                ? `<span class="badge badge-live">LÃƒÂ¤uft</span>`
...
function summarizeStageProfiles(stages) {
    if (stages.length === 0) {
    ...
    return Array.fürom(counts.entries())
        ...
        .map(([profile, count]) => `${count}ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${profile}`)
        .join(' · ');
}
function getStageProfileClassName(profile) {
    return `stage-profile-badge-${profile.toLowerCase().replace(/_/g, '-')}`;
}
function renderStageProfileBadge(profile) {
    return `<span class="stage-profile-badge ${getStageProfileClassName(profile)}">${esc(profile)}</span>`;
}
function buildDashboardStageProfileLabel(race, stage) {
    return `${race.name} · ${getStageDisplayName(stage)} · ${stage.profile}`;
}
async function ensureStageSummaryLoaded(stageId) {
...
function renderDashboardRaceStagesModal() {
...
async function openDashboardRaceStages(raceId) {
...
function renderProgramRaceRows(payload) {
...
async function openRiderProgram(riderId) {
...
function renderRaceParticipantRows(participants) {
...
async function openRaceProgramParticipants(raceId) {
    const race = findRaceById(raceId);
...
async function refüreshRaceProgramParticipants(showLoadingState = false) {
...
async function openDashboardStageProfile(stageId, selectedClimb = null) {
    const location = findStageById(stageId);
    if (!location) {
        return;
    }
    ...
    renderStaticStageProfile($('stage-profile-view'), summary, location.stage.profile, buildDashboardStageProfileLabel(location.race, location.stage), selectedClimb != null
...
async function refüreshTeamsViewData() {
...
function rebuildRiderTeamEditorTeams(payload) {
...
async function loadRiderTeamEditorData(force = false) {
...
function updateRiderTeamEditorField(riderId, field, rawValue) {
    const payload = state.riderTeamEditorPayload;
    if (!payload) {
        return;
    }
    ...
    if (field === 'teamId') {
        ...
        rider[field] = Number.parseInt(rawValue || '0', 10);
    }
    else {
        rider[field] = rawValue;
    }
...
async function saveRiderTeamEditorData() {
...
async function exportRiderTeamEditorData() {
...
function renderRidersMenu() {
    ...
    <div class="team-detail-card">
      ...
      <div class="team-detail-meta riders-pagination" style="margin-top:0.75rem">
        <button type="button" class="btn btn-secondary btn-sm" data-riders-page-action="prev" ${state.riderMenuPage <= 1 ? 'disabled' : ''}>ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â Ãƒâ€šÃ‚Â Zurück</button>
        <span>Seite ${state.riderMenuPage} / ${totalPages} · Fahrer ${totalRiders === 0 ? 0 : startIndex + 1}-${endIndex} von ${totalRiders}</span>
        <button type="button" class="btn btn-secondary btn-sm" data-riders-page-action="next" ${state.riderMenuPage >= totalPages ? 'disabled' : ''}>Weiter ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢</button>
      </div>
    </div>`;
}
...
const RIDER_STATS_ICONS = {
    ...
    breakaway: '<span class="rider-stats-breakaway-icon" aria-label="AusreiÃƒÆ’Ã…Â¸er" title="AusreiÃƒÆ’Ã…Â¸er" style="margin-right: 0.4rem;">??</span>',
};
function renderRiderOverallRatingBadge(score) {
    const minScore = 50;
    const maxScore = 85;
...
function renderRiderStatsIconHeatPill(icon, title, value, maxValue) {
    const ratio = maxValue > 0 ? Math.max(0, Math.min(1, value / maxValue)) : 0.5;
    const hue = Math.round(6 + (ratio * 118));
    const borderAlpha = 0.26 + (ratio * 0.18);
    const bgAlpha = 0.14 + (ratio * 0.12);
...
function renderRiderStatsSummary(rider, payload, teamName, countryCode, countryFlag) {
