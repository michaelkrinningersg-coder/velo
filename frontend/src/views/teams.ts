...
import {
  ...
  renderCountry,
  renderMiniJersey,
  renderRiderAvailabilityMarker,
  formatRiderName,
  getRiderCountryCode,
  getRiderRoleName,
  renderSkillValue,
  renderSkillValueWithDelta,
  renderRaceFormBonusValue,
  renderSeasonFormValue,
  renderLoadMalusValue,
  renderSeasonFormPhase,
  renderSeasonFormPhaseIndicator,
  renderRiderProgramButton,
  isActiveView,
  showLoading,
  hideLoading,
...
import { renderRiderNameLink, resolveRaceCategoryBadgeStyle } from '../riderStatsUi';

// Dynamically or directly imported view functions
import { openRiderProgram } from './dashboard';

export const TEAM_SKILL_COLUMNS: Array<{ key: keyof Rider['skills']; label: string }> = [
  { key: 'flat', label: 'Fl' },
...
export const TEAM_DETAIL_PAGE_ORDER: TeamDetailPage[] = ['skills', 'form', 'profile', 'preferences'];
...
export const TEAM_DETAIL_PAGE_COLUMNS: Record<TeamDetailPage, TeamTableColumn[]> = {
  skills: [
    ...
    ...TEAM_SKILL_COLUMNS.map((column) => ({
      ...
      label: column.label,
      title: column.label,
      sortKey: column.key as TeamTableSortKey,
...
export function getActiveTeamTableColumns(): TeamTableColumn[] {
  return [...TEAM_TABLE_COLUMNS, ...TEAM_DETAIL_PAGE_COLUMNS[state.teamDetailPage]];
}

export function getTeamTopAverage(teamId: number, limit = 12): number | null {
...
export function getTeamAverage(teamId: number): number | null {
...
export function getPeakDate(rider: Rider, index: number): string | undefined {
  return rider.seasonFormPeakDates?.[index];
}

export function getSpecializationSortLabel(value: any): string | undefined {
...
export function getSortIndicator(sortKey: TeamTableSortKey): string {
...
export function renderTeamTableHeader(column: TeamTableColumn): string {
  if (!column.sortKey) {
...
export function renderTeamDetailPageTabs(): string {
  return `
...
export const TEAM_SKILL_TITLES: Record<string, string> = {
  flat: 'Flach',
  mountain: 'Berg',
...
export function getTeamSortLabel(sortKey: TeamTableSortKey): string {
  if (sortKey === 'name') return 'Name';
  if (sortKey === 'countryCode') return 'Herkunft';
  if (sortKey === 'birthYear') return 'Geburtsjahr';
  if (sortKey === 'age') return 'Alter';
  if (sortKey === 'overallRating') return 'Stärke';
  if (sortKey === 'potOverall') return 'Potenzial';
  if (sortKey === 'formBonus') return 'Saisonform';
  if (sortKey === 'raceFormBonus') return 'Rennform';
  if (sortKey === 'averageForm') return 'Average Form';
  if (sortKey === 'longTermFatigueMalus') return 'Langzeiterschöpfung';
  if (sortKey === 'shortTermFatigueMalus') return 'Akute Erschöpfung';
  ...
  if (sortKey === 'specialization2') return 'Spec 2';
  if (sortKey === 'specialization3') return 'Spec 3';
  if (sortKey === 'skillDevelopment') return 'Entwicklung';
  if (sortKey === 'mentorName') return 'Mentor';
...
export function sortTeamRiders(riders: Rider[]): Rider[] {
  ...
  sortedRiders.sort((left, right) => {
    ...
    switch (state.teamTableSort.key) {
      ...
      case 'peak3':
        ...
        break;
      default:
        comparison = left.skills[state.teamTableSort.key as keyof Rider['skills']] - right.skills[state.teamTableSort.key as keyof Rider['skills']];
        break;
    }
...
export function sortRiderMenuRiders(riders: Rider[]): Rider[] {
  const sortedRiders = [...riders];
  ...
  sortedRiders.sort((left, right) => {
    ...
    switch (state.riderMenuTableSort.key) {
      ...
      case 'peak3':
        ...
        break;
      default:
        comparison = left.skills[state.riderMenuTableSort.key as keyof Rider['skills']] - right.skills[state.riderMenuTableSort.key as keyof Rider['skills']];
        break;
    }
...
export function renderRacePrefs(raceIds: number[]): string {
...
export function renderPeakDatesSummary(rider: Rider): string {
...
export function renderTeamTableCell(rider: Rider, column: TeamTableColumn): string {
  switch (column.id) {
    case 'name':
...
export async function refreshTeamsViewData(): Promise<void> {
  ...
  try {
    ...
    await api.getTeams().then(t => {
      if (t.success) state.teams = t.data ?? [];
    });
...
export async function loadTeams(options: { render?: boolean } = {}): Promise<void> {
...
export function renderTeamDetail(teamId: number | null): void {
...
export function initTeamsListeners(): void {
