...
export const api = {
  ...
  getRivalries:        (season?: number) => call<RivalryOverviewPayload>('GET', `/api/rivalries${season != null ? `?season=${season}` : ''}`),
  getRivalryDetail:    (aId: number, bId: number, season?: number) => call<RivalryDetailPayload>('GET', `/api/rivalries/${aId}/${bId}${season != null ? `?season=${season}` : ''}`),
  getSeasonWrapped:    (season: number) => call<SeasonWrappedPayload>('GET', `/api/season-wrapped/${season}`),
  ...
  saveContractRenewalSelection: (riderIds: number[]) => call<any>('POST', '/api/contract-renewals/select', { riderIds }),
